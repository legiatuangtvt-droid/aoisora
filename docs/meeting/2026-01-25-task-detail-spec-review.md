...
**Navigation Flow (ĐÃ CẬP NHẬT THEO DEV TEAM):**

```
┌─────────────────────────────────────────────────────────────────┐
│  ⚠️ CLARIFICATION TỪ DEV TEAM (2026-01-25):                      │
│                                                                 │
│  • Task Cha KHÔNG có route /tasks/{id} riêng                    │
│  • Task Cha chỉ expand/collapse TRONG Task List screen          │
│  • Chỉ Task Lá mới có thể navigate đến /tasks/{id}              │
│                                                                 │
│  TASK LIST (/tasks/list)                                        │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ Kiểm kê Q1 2026 [▼]          ← CHA: expand/collapse      │    │
│  │   Kiểm kê Thực phẩm [▶]     ← CHA: expand/collapse       │    │
│  │   Kiểm kê Điện máy          ← LÁ: click → /tasks/5       │    │
│  │   Kiểm kê Thời trang        ← LÁ: click → /tasks/6       │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│  /tasks/5 (LEAF - Task Lá)                                      │
...
### 6.4 Parent-Child Task Display Logic (MỚI - ĐÃ CẬP NHẬT)

> ⚠️ **CLARIFICATION TỪ DEV TEAM (2026-01-25):**
> - **Chỉ có Task Lá mới có thể vào Detail screen**
> - **Task Cha KHÔNG có Detail page riêng** - chỉ hiển thị trong Task List với expand/collapse
> - **Status của Task Cha được AUTO-CALCULATE** khi tất cả Task Con được check

**QUAN TRỌNG: Task Cha vs Task Lá trong UI**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  QUY TẮC NAVIGATION (ĐÃ CHỐT VỚI DEV TEAM)                                  │
│                                                                             │
│  1. TASK CHA (có child tasks):                                              │
│     → KHÔNG CÓ Detail page riêng                                            │
│     → Chỉ hiển thị trong Task List với icon ▶/▼ để expand/collapse          │
│     → Click vào row → Expand/Collapse children (KHÔNG navigate)             │
│     → Status được AUTO-CALCULATE từ tất cả Task Con                         │
│                                                                             │
│  2. TASK LÁ (không có child tasks):                                         │
│     → CÓ Detail page với Store Cards, HQ Check, Evidence                    │
│     → Click vào row → Navigate đến /tasks/{task_id}                         │
│     → Status được tính từ store assignments                                 │
│                                                                             │
│  3. AUTO-CALCULATION STATUS CỦA TASK CHA:                                   │
│     → Khi TẤT CẢ Task Con = done/unable → Task Cha = done                   │
│     → Khi ÍT NHẤT 1 Task Con = on_progress → Task Cha = on_progress         │
│     → Khi TẤT CẢ Task Con = not_yet → Task Cha = not_yet                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**Task List UI với Expand/Collapse:**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│ TASK LIST - HIERARCHICAL VIEW (CÓ EXPAND/COLLAPSE)                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│ ┌─────────┬──────────────────────────┬─────────────┬──────────┐             │
│ │ Lvl     │ Task Name                │ Status      │ Progress │             │
│ ├─────────┼──────────────────────────┼─────────────┼──────────┤             │
│ │ 1 [▼]   │ Kiểm kê Q1 2026          │ 🔵 Progress │ -        │ ← CHA       │
│ │ 1.1 [▶] │ Kiểm kê Thực phẩm        │ 🔵 Progress │ -        │ ← CHA       │
│ │ 1.2     │ Kiểm kê Điện máy         │ 🔵 Progress │ 30/50    │ ← LÁ ✓      │
│ │ 1.3     │ Kiểm kê Thời trang       │ ⚪ Not Yet  │ 0/50     │ ← LÁ ✓      │
│ └─────────┴──────────────────────────┴─────────────┴──────────┘             │
│                                                                             │
│ 💡 LEGEND:                                                                  │
│    • [▼]/[▶] = Task CHA (click icon để expand/collapse, KHÔNG vào Detail)  │
│    • Không có icon = Task LÁ (click row để vào Detail page)                 │
│    • Progress = "-" → Task CHA                                              │
│    • Progress = "30/50" → Task LÁ                                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**CHỈ Task Lá có Detail page (như Case 3 dưới đây):**
...
**Tóm tắt Navigation Rules (ĐÃ CẬP NHẬT):**

| Loại Task | Có Child? | Click Action | Detail Page? |
|-----------|-----------|--------------|--------------|
| Task CHA | ✅ Có | Expand/Collapse children trong Task List | ❌ KHÔNG có |
| Task LÁ | ❌ Không | Navigate đến /tasks/{task_id} | ✅ CÓ |
...
### 6.5 Hierarchical Table View (Bảng tổng quan cấu trúc Task)

> ⚠️ **CLARIFICATION TỪ DEV TEAM (2026-01-25):**
> - Task Cha chỉ expand/collapse trong Task List, KHÔNG có Detail page
> - Chỉ Task Lá mới có thể click để vào Detail page

**Hierarchical Table với Navigation Rules:**
...
│ │ 1       │ Kiểm kê Q1 2026          │ 🔵 Progress │ -        │ ← CHA   │
│ │ 1.1     │ Kiểm kê Thực phẩm        │ 🔵 Progress │ -        │ ← CHA   │
│ │ 1.1.1   │ Kiểm kê Rau củ           │ ✅ Done     │ 25/25    │ ← LÁ    │
│ │ 1.1.2   │ Kiểm kê Hải sản          │ 🔵 Progress │ 10/25    │ ← LÁ    │
│ │ 1.2     │ Kiểm kê Điện máy         │ 🔵 Progress │ 30/50    │ ← LÁ    │
│ │ 1.3     │ Kiểm kê Thời trang       │ ⚪ Not Yet  │ 0/50     │ ← LÁ    │
│ └─────────┴──────────────────────────┴─────────────┴──────────┘         │
│                                                                         │
│ 💡 LEGEND:                                                              │
│    • Progress = "-"       → Task CHA (có child tasks)                   │
│    • Progress = "25/25"   → Task LÁ (giao trực tiếp cho stores)         │
│                                                                         │
│ 📍 NAVIGATION RULES (ĐÃ CHỐT):                                          │
│    • Task CHA (Progress = "-") → Click expand/collapse, KHÔNG vào Detail│
│    • Task LÁ (Progress = số) → Click row để navigate đến Task Detail    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| Progress | Loại Task | Click Action | Detail Page? |
|----------|-----------|--------------|--------------|
| `-` | Task CHA | Expand/Collapse children | ❌ KHÔNG |
| `25/25`, `30/50` | Task LÁ | Navigate đến `/tasks/{task_id}` | ✅ CÓ |

> **Lưu ý:** Task Cha chỉ expand/collapse trong Task List. User phải click vào Task Lá để xem Detail page với Store Cards.
...
│ Lvl       │ Task Name                  │ Status      │ Progress      │
├───────────┼────────────────────────────┼─────────────┼───────────────┤
│ 1         │ Kiểm kê Năm 2026           │ 🔵 Progress │ -         CHA │
│ 1.1       │ Kiểm kê Q1                 │ 🔵 Progress │ -         CHA │
│ 1.1.1     │ Kiểm kê Thực phẩm          │ 🔵 Progress │ -         CHA │
│ 1.1.1.1   │ Rau củ                     │ ✅ Done     │ 25/25     LÁ  │
│ 1.1.1.2   │ Hải sản                    │ 🔵 Progress │ 10/25     LÁ  │
│ 1.1.2     │ Kiểm kê Điện máy           │ 🔵 Progress │ 30/50     LÁ  │
│ 1.1.3     │ Kiểm kê Thời trang         │ ⚪ Not Yet  │ 0/50      LÁ  │
│ 1.2       │ Kiểm kê Q2                 │ ⚪ Not Yet  │ -         CHA │
│ 1.2.1     │ (same structure...)        │             │               │
│ ...       │                            │             │               │
└───────────┴────────────────────────────┴─────────────┴───────────────┘

💡 NAVIGATION RULES:
   • Progress = "-" (CHA) → Click để expand/collapse, KHÔNG vào Detail
   • Progress = số (LÁ) → Click row để navigate đến /tasks/{task_id}
```
...
├─────────────────────────────────────────────────────────────────────────────────────────┤
│ OPTION B: Collapsible View (Click expand/collapse icon để mở/đóng children)             │
│                                                                                         │
│ ┌─────────┬──────────────────────────┬─────────────┬──────────┐                         │
│ │ Lvl     │ Task Name                │ Status      │ Progress │                         │
│ ├─────────┼──────────────────────────┼─────────────┼──────────┤                         │
│ │ 1 [▼]   │ Kiểm kê Q1 2026          │ 🔵 Progress │ -        │  ← CHA (expanded)       │
│ │ 1.1 [▶] │ Kiểm kê Thực phẩm        │ 🔵 Progress │ -        │  ← CHA (collapsed)      │
│ │ 1.2     │ Kiểm kê Điện máy         │ 🔵 Progress │ 30/50    │  ← LÁ                   │
│ │ 1.3     │ Kiểm kê Thời trang       │ ⚪ Not Yet  │ 0/50     │  ← LÁ                   │
│ └─────────┴──────────────────────────┴─────────────┴──────────┘                         │
│                                                                                         │
│ → [▼] = Expanded (đang mở, hiển thị children bên dưới) - TASK CHA                       │
│ → [▶] = Collapsed (đang đóng, click icon hoặc row để expand) - TASK CHA                 │
│ → Không có icon = Leaf task (không có children) - TASK LÁ                               │
│                                                                                         │
│ 📍 NAVIGATION (ĐÃ CHỐT VỚI DEV TEAM):                                                   │
│ → Task CHA ([▼]/[▶]): Click row = Expand/Collapse, KHÔNG navigate đến Detail           │
│ → Task LÁ (không icon): Click row = Navigate đến /tasks/{task_id} để xem Detail        │
│                                                                                         │
└─────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 5 | ✅ Update spec - Navigation rules (Task Cha không có Detail) | Claude | 2026-01-25 |
| 6 | Implement Phase 1 | Dev Team | - |
| 7 | ❌ ~~Update API response cho parent tasks~~ | Dev Team | N/A (Task Cha không cần API Detail) |
