...
#### Sơ đồ tổng quan luồng Task (Complete Task Status Flow)
...
│                                       ▼                                                    │
│                                ┌─────────────┐                                             │
│                                │   NOT YET   │◄── TASK STATUS (ít nhất 01 stores = not_yet)│
│                                └──────┬──────┘                                             │
│  ┌────────────────────────────────────┼─────────────────────────┬────────────────────┐     │
│  │                                    │                         │     ON PROGRESS    │     │
│  │                                    ▼                         │ (ít nhất 1 store   │     │
...
│    → Library mark: Đánh dấu task tương ứng trong Library là "had_issues"                   │
│    → Tại APPROVE: Approver có thể SỬA thông tin task                                       │
...
│  │ unable              │                                                        │          │
│  ├─────────────────────┼────────────────────────────────────────────────────────┤          │
│  │ not_yet → overdue   │ SYSTEM AUTO: today > end_date (chưa bắt đầu)           │          │
│  ├─────────────────────┼────────────────────────────────────────────────────────┤          │
│  │ on_progress →       │ SYSTEM AUTO: today > end_date (đang làm dở)            │          │
│  │ overdue             │                                                        │          │
│  ├─────────────────────┼────────────────────────────────────────────────────────┤          │
...
│  ══════════════════════════════════════════════════════════════════════════════════════════│
│                                                                                            │
│  THỨ TỰ ƯU TIÊN: overdue > not_yet > on_progress > done                                   │
│                                                                                            │
│  ┌─────────────────────┬────────────────────────────────────────────────────────┐          │
...
│  │ Case 2: A=on_progress, B=not_yet, C=not_yet     → TASK = NOT YET (có not_yet)  │       │
│  │ Case 3: A=done_pending, B=not_yet, C=not_yet    → TASK = NOT YET (có not_yet)  │       │
...
│  │ Case 9: A=done, B=not_yet, C=overdue            → TASK = OVERDUE ⚠️            │       │
│  └─────────────────────────────────────────────────────────────────────────────────┘       │
│                                                                                            │
...
│                                                                                            │
│  ══════════════════════════════════════════════════════════════════════════════════════════│
│  📌 FINAL RESULT COUNTERS (Bộ đếm kết quả cuối cùng):                                      │
│  ══════════════════════════════════════════════════════════════════════════════════════════│
│                                                                                            │
│    Khi task hoàn thành (status = DONE hoặc OVERDUE), kết quả hiển thị qua 2 bộ đếm:       │
│                                                                                            │
│    ┌─────────────────────────────────────────────────────────────────────────────────┐     │
│    │  1️⃣ UNABLE COUNTER (Bộ đếm Unable)                                               │     │
│    │  ────────────────────────────────────────────────────────────────────────────── │     │
│    │  • Hiển thị: Số nguyên (ví dụ: 5)                                               │     │
│    │  • Ý nghĩa: Số stores đã báo "unable" (không thể thực hiện)                     │     │
│    │  • Bộ đếm riêng biệt, không nằm trong done counter                              │     │
│    │                                                                                 │     │
│    │  2️⃣ DONE COUNTER (Bộ đếm Done)                                                   │     │
│    │  ────────────────────────────────────────────────────────────────────────────── │     │
│    │  • Hiển thị: X/Y format (ví dụ: 24/27)                                          │     │
│    │  • X = Số stores đã done (confirmed)                                            │     │
│    │  • Y = Tổng số stores còn lại (Total - unable)                                  │     │
│    │  • Y - X = Số stores overdue (chưa hoàn thành khi hết hạn)                      │     │
│    │                                                                                 │     │
│    └─────────────────────────────────────────────────────────────────────────────────┘     │
│                                                                                            │
│    📊 VÍ DỤ:                                                                               │
│    ┌─────────────────────────────────────────────────────────────────────────────────┐     │
│    │  Task giao cho 32 stores, kết quả cuối cùng:                                    │     │
│    │                                                                                 │     │
│    │  ┌───────────────────────────────────────────────────────────────────────────┐  │     │
│    │  │  done   │  24/27  │  24 stores done, 27 tổng còn lại, 3 overdue (27-24=3) │  │     │
│    │  ├─────────┼─────────┼───────────────────────────────────────────────────────┤  │     │
│    │  │  unable │    5    │  5 stores báo unable (riêng biệt)                     │  │     │
│    │  └─────────┴─────────┴───────────────────────────────────────────────────────┘  │     │
│    │                                                                                 │     │
│    │  📌 CÁCH TÍNH:                                                                  │     │
│    │     → Total stores assigned = 32                                                │     │
│    │     → 5 stores báo unable → unable counter = 5                                  │     │
│    │     → Còn lại 27 stores (32 - 5) được track trong done counter                  │     │
│    │     → Trong 27 stores đó: 24 done, 3 overdue                                    │     │
│    │     → done counter hiển thị: 24/27                                              │     │
│    │                                                                                 │     │
│    │  ✅ FORMULA:                                                                     │     │
│    │     Total = done + overdue + unable                                             │     │
│    │     32    = 24   + 3       + 5                                                  │     │
│    │                                                                                 │     │
│    │     done counter: X/Y                                                           │     │
│    │     → Y = Total - unable = 32 - 5 = 27                                          │     │
│    │     → X = số stores done = 24                                                   │     │
│    │     → overdue = Y - X = 27 - 24 = 3                                             │     │
│    │                                                                                 │     │
│    └─────────────────────────────────────────────────────────────────────────────────┘     │
...
|---|-------|----------|----------|-------------------|
| 1 | G2-G9 | HQ Creator | Đăng nhập → Task List → Add New → Điền thông tin → Chọn Stores → Submit | Login, Task List, Add New |
| 2 | G3-G9 | HQ Approver | Đăng nhập → Nhận notification → Task List → Add New (Approval mode) → Review → Approve/Reject | Login, Task List, Add New |
...
| 6 | G2-G9 | HQ Creator | Đăng nhập → Library → Add New → Điền thông tin template → Submit | Login, Library, Add New |
| 7 | G3-G9 | HQ Approver | Đăng nhập → Nhận notification → Library → Add New (Approval mode) → Review → Approve → Available | Login, Library, Add New |
| 8 | G2-G9 | HQ User | Đăng nhập → Library → Chọn template (Available) → Add New → Chọn Scope → Gửi | Login, Library, Add New |
...
│                    ▼                 ▼                 ▼                       │
│             ┌───────────┐     ┌───────────┐     ┌───────────┐                  │
│             │on_progress│     │done_pending│    │  unable   │                  │
│             └─────┬─────┘     └─────┬─────┘     └───────────┘                  │
│                   │                 │                                          │
│                   │                 ▼                                          │
│                   │          ┌───────────┐                                     │
