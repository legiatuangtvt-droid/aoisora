...
| 2 | G3-G9 | HQ Approver | Nhận notification → Approval screen → Review → Approve/Reject | Approval |
| 3 | S2-S4 | Store Leader | Nhận notification → Store Tasks → Xem chi tiết → Assign cho Staff hoặc tự làm | Store Tasks |
| 4 | S1 | Store Staff | Xem "My Tasks" → Thực hiện → Upload evidence → Mark Done | Store Tasks |
| 5 | G2-G9 | HQ Checker | HQ Check screen → Xem evidence → Checked/Reject | HQ Check |
| 6 | G2-G9 | HQ User | Library → Chọn template → Dispatch → Chọn Scope → Gửi | Library, Dispatch |
