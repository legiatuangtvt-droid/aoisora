...
| Must Edit After Reject | Phải sửa ít nhất 1 field trước khi submit lại | Bắt buộc |
...
| Rule | Mô tả |
|------|-------|
| Auto-Save to Library | Khi task được approve, tự động lưu vào Library |
| Cooldown Trigger | Khi dispatch cùng template + cùng stores + cùng period |
| Cooldown Duration | Từ start_date đến end_date của task đã dispatch |
| Override Cooldown | Chỉ Dept/Team Head (highest grade) có quyền |

### 7.4 Overdue Rules

| Rule | Mô tả |
|------|-------|
| Overdue Detection | end_date < today VÀ status không phải done |
| Auto Check | System chạy daily hoặc on-demand |
| Auto Confirm | done_pending + overdue → done (auto confirm) |
