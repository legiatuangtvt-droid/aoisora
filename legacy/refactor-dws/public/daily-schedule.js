...
import { initializeCheckTaskView, revertToMainSchedule } from './check-task.js'
...
async function handleTaskClick(event) {
  ...
  if (isTryingToComplete) {
    ...
    const allowedStartTime = new Date(
      taskStartDateTime.getTime() - 60 * 60 * 1000
    ) // 1 giờ = 60 phút * 60 giây * 1000 ms

    // So sánh với thời gian hiện tại
    if (new Date() < allowedStartTime) {
      window.showToast(
        'Chỉ có thể xác nhận hoàn thành trước tối đa 1 giờ.',
        'warning'
      )
  ...
  if (isCurrentlyCompleted) {
    ...
    const timeDiffMinutes = Math.abs(
      (now.getTime() - taskStartDateTime.getTime()) / (1000 * 60)
    )
  ...
  try {
    ...
    const userToUpdateId = !isCurrentlyCompleted
      ? completingUserId
      : taskForPoints.completingUserId || completingUserId
    ...
    if (employeeToUpdate) {
      employeeToUpdate.experiencePoints =
        (employeeToUpdate.experiencePoints || 0) + pointsChange
    }
...
export async function init() {
  ...
  document
    ...
    ?.addEventListener('click', () => changeWeek(-1), { signal })
  document
  ...
  if (window.currentUser?.roleId === 'STAFF') {
    ...
    checkTaskBtn.addEventListener(
      'click',
      () => {
        if (checkTaskBtn.classList.contains('dws-active')) {
