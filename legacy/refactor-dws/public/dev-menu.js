...
function initializeDevMenu() {
  ...
  async function fetchPersonnelData() {
    ...
    try {
      ...
      allEmployees = employeesSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }))
      allAreaManagers = areaManagersSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }))
      allRegionalManagers = regionalManagersSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }))
      ...
      const storedSimulatedUser = localStorage.getItem(
        SIMULATED_USER_STORAGE_KEY
      )
      if (storedSimulatedUser) {
        ...
        if (simulatedUser && simulatedUser.id && simulatedUser.roleId) {
          ...
          window.showToast(
            `Đang mô phỏng: ${simulatedUser.name} (${simulatedUser.roleId})`,
            'info'
          )
        }
      } else {
  ...
  function populateEmployeeSelect(selectedRoleId = 'all') {
    ...
    const optionsHtml = filteredPersonnel
      .map(person => {
        let contextInfo = ''
        switch (person.roleId) {
          ...
          case 'STORE_INCHARGE': {
            if (
              Array.isArray(person.managedStoreIds) &&
              person.managedStoreIds.length > 0
            ) {
              const managedStoreNames = person.managedStoreIds
                .map(id => allStores.find(s => s.id === id)?.name)
                .filter(Boolean)
              if (managedStoreNames.length > 0)
                contextInfo = ` - (Quản lý: ${managedStoreNames.join(', ')})`
            }
            break
          }
          case 'AREA_MANAGER': {
  ...
  employeeSelect.addEventListener('change', () => {
    ...
    if (selectedEmployeeId) {
      ...
      if (selectedPerson) {
        // Lưu toàn bộ thông tin cần thiết của người dùng mô phỏng
        simulatedUser = {
        ...
        window.showToast(
          `Chuyển sang người dùng: ${selectedPerson.name} (${selectedPerson.roleId})`,
          'info'
        )
  ...
  seedAllDataBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const data = allDataArray.reduce(
      ...
      for (const collName of collectionsToDelete) {
        ...
        if (
          data[collName] === undefined &&
          !['staff', 'staff_statuses', 'monthly_plans'].includes(collName)
        )
          continue
      ...
      window.showToast(
        'Hoàn tất! Đã nhập toàn bộ dữ liệu mẫu.',
        'success',
        4000
      )
    } catch (error) {
      ...
      window.showToast(
        'Lỗi khi nhập dữ liệu. Vui lòng kiểm tra console.',
        'error'
      )
    }
  })
...
async function applyTemplateToAllStores() {
  ...
  try {
    ...
    const templatesSnap = await getDocs(
      query(collection(db, 'daily_templates'), orderBy('name'))
    )
    const allTemplates = templatesSnap.docs.map(d => ({
      id: d.id,
      ...d.data(),
    }))
    ...
    const defaultTemplateName =
      allTemplates.length > 0 ? allTemplates[0].name : ''
    ...
    const template = allTemplates.find(
      t => t.name.toLowerCase() === templateName.toLowerCase().trim()
    )
    if (!template || !template.schedule || !template.shiftMappings) {
      throw new Error(
        `Template "${templateName}" không hợp lệ hoặc không có dữ liệu ca làm việc.`
      )
    }
    ...
    const allWorkPositions = workPositionsSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
    const scheduleHistory = historySnap.docs.map(doc => doc.data())
    const allShiftCodes = shiftCodesSnap.exists()
      ? shiftCodesSnap.data().codes
      : []
    ...
    scheduleHistory.forEach(schedule => {
      ...
      if (currentHoursMap.has(positionName)) {
        currentHoursMap.set(
          positionName,
          currentHoursMap.get(positionName) + shiftInfo.duration
        )
      }
    })

    window.showToast(
      `Bắt đầu tạo lịch cho ngày ${dateString} từ mẫu "${template.name}"...`,
      'info'
    )
    ...
    if (!oldSchedulesSnap.empty) {
      window.showToast(
        `Đã xóa ${oldSchedulesSnap.size} lịch làm việc cũ của ngày ${dateString}.`,
        'info'
      )
    }
    ...
    for (const store of allStores) {
      const storeEmployees = allEmployees.filter(
        emp => emp.storeId === store.id && emp.status === 'ACTIVE'
      )
      ...
      const leaderSlots = slotsToFill.filter(
        slot => slot.positionId === 'LEADER' && !slot.employeeId
      )
      const leadersInStore = availableEmployees.filter(emp =>
        emp.roleId.includes('LEADER')
      )

      if (leaderSlots.length > 0 && leadersInStore.length > 0) {
        let leaderIndex = 0
        for (const slot of leaderSlots) {
          const leaderToAssign =
            leadersInStore[leaderIndex % leadersInStore.length]
      ...
      const employeeAvailabilities = allAvailabilities.filter(a =>
        availableEmployees.some(e => e.id === a.employeeId)
      )
      // Sắp xếp nhân viên theo priority (1 là cao nhất)
      employeeAvailabilities.sort(
        (a, b) =>
          (a.registrations[0]?.priority || 3) -
          (b.registrations[0]?.priority || 3)
      )

      for (const availability of employeeAvailabilities) {
        // Nếu nhân viên này đã được phân công, bỏ qua
        if (!availableEmployees.some(e => e.id === availability.employeeId))
          continue
        ...
        for (const registration of availability.registrations) {
          ...
          const matchingSlot = slotsToFill.find(
            slot =>
          ...
          if (matchingSlot) {
            ...
            availableEmployees = availableEmployees.filter(
              emp => emp.id !== availability.employeeId
            )
      ...
      for (const slot of remainingSlots) {
        if (availableEmployees.length === 0) {
          ...
          console.warn(
            `Cửa hàng ${store.name} không đủ nhân viên để lấp đầy suất ${slot.positionId} (${slot.shiftCode})`
          )
          continue
        }
        ...
        availableEmployees.sort((a, b) => {
          ...
          const getShortage = employeeId => {
            ...
            const totalHours = positionHours
            ...
            const currentPercentage =
              totalHours > 0
                ? (positionHours?.get(slot.positionId) || 0) / totalHours
                : 0
      ...
      for (const filledSlot of slotsToFill) {
        ...
        if (filledSlot.employeeId) {
          const tasks = (templateSchedule[filledSlot.shiftId] || []).map(
            task => ({
              // Áp dụng giá trị isComplete
    ...
    await batch.commit()
    window.showToast(
      `Hoàn tất! Đã tạo ${schedulesCreatedCount} lịch làm việc cho ngày ${dateString} tại ${allStores.length} cửa hàng.`,
      'success',
      5000
    )
  } catch (error) {
...
async function reCalculateAllEmployeeExp() {
  ...
  try {
    ...
    window.showToast(
      `... (3/3) Đang cập nhật ${employeeExpMap.size} nhân viên...`,
      'info'
    )
    ...
    employeeExpMap.forEach((totalExp, employeeId) => {
      ...
      updatePromises.push(
        updateDoc(employeeRef, { experiencePoints: totalExp })
      )
    })
    await Promise.all(updatePromises)

    window.showToast(
      '🎉 HOÀN TẤT! Đã cập nhật thành công điểm kinh nghiệm.',
      'success',
      5000
    )
  } catch (error) {
    ...
    window.showToast(
      'Đã có lỗi xảy ra. Vui lòng kiểm tra Console (F12).',
      'error'
    )
  } finally {
    btn.disabled = false
