...
export function calculateREForTask(task, groupInfo, reParameters) {
  ...
  const totalCustomerCount = reParameters?.customerCountByHour
    ? Object.values(reParameters.customerCountByHour).reduce(
        (sum, count) => sum + count,
        0
      )
  ...
  switch (groupInfo.code) {
    ...
    case 'DRY': {
      const dryTasksWithVolume = [
        'Lên hàng khô',
        'Kéo mặt hàng khô',
        'Check tồn WH nóc kệ châm hàng',
        'Kéo mặt Grocery',
        'Kéo mặt NF-HBC HL-SL',
      ]
