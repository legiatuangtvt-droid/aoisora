...
import {
  ...
  showLoadingSpinner,
  setIsInitialLoad,
  startAttentionAnimationInterval,
} from './daily-schedule-ui.js'
...
export async function fetchInitialData() {
  try {
    ...
    const workPositionsQuery = query(
      collection(db, 'work_positions'),
      where('status', '==', 'ACTIVE')
    )
    const [
      shiftCodesSnap,
    ...
    allWorkPositions = workPositionsSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))

    let fetchedEmployees = employeesSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
      ...
      switch (currentUser.roleId) {
        ...
        case 'STORE_LEADER':
          fetchedEmployees = fetchedEmployees.filter(
            emp => emp.storeId === currentUser.storeId
          )
          break
        case 'AREA_MANAGER':
          if (
            currentUser.managedAreaIds &&
            currentUser.managedAreaIds.length > 0
          ) {
            const managedStoreIds = allStores
              .filter(s => currentUser.managedAreaIds.includes(s.areaId))
              .map(s => s.id)
            fetchedEmployees = fetchedEmployees.filter(emp =>
              managedStoreIds.includes(emp.storeId)
            )
          }
          break
        case 'REGIONAL_MANAGER':
          if (currentUser.managedRegionId) {
            const managedAreaIds = allAreas
              .filter(a => a.regionId === currentUser.managedRegionId)
              .map(a => a.id)
            const managedStoreIds = allStores
              .filter(s => managedAreaIds.includes(s.areaId))
              .map(s => s.id)
            fetchedEmployees = fetchedEmployees.filter(emp =>
              managedStoreIds.includes(emp.storeId)
            )
...
export function listenForScheduleChanges(dateString) {
  ...
  window.currentScheduleUnsubscribe = onSnapshot(
    scheduleQuery,
    querySnapshot => {
      const schedules = []
      querySnapshot.forEach(doc => {
        const scheduleData = doc.data()
        const employeeInfo = allEmployees.find(
          emp => emp.id === scheduleData.employeeId
        )
        ...
        currentScheduleData.sort((a, b) => {
          ...
          const getStartTime = shiftCode => {
            const shiftInfo = allShiftCodes.find(
              sc => sc.shiftCode === shiftCode
            )
            return shiftInfo
              ? shiftInfo.timeRange.split('~')[0].trim()
              : '99:99' // Default to a late time if not found
          }
          ...
          const orderA = Object.keys(dailyTemplate.shiftMappings).findIndex(
            key => dailyTemplate.shiftMappings[key].positionId === a.positionId
          )
          const orderB = Object.keys(dailyTemplate.shiftMappings).findIndex(
            key => dailyTemplate.shiftMappings[key].positionId === b.positionId
          )
...
export function changeWeek(direction) {
  ...
  const selectedDateString = activeDayButton
    ? activeDayButton.dataset.date
    : null
  renderWeekControls(selectedDateString) // Truyền ngày đang active vào
  // Tự động chọn ngày tương ứng ở tuần mới và tải lịch
  const newDayToSelect =
    document.querySelectorAll('.day-selector-btn')[selectedDayIndex]
  if (newDayToSelect) {
 ...
 */
export function changeDay(dateString = false) {
...
export async function updateTaskStatus(
  ...
  await runTransaction(db, async transaction => {
    ...
    const userToUpdateId = newIsComplete
      ? completingUserId
      : targetTask.completingUserId || completingUserId
