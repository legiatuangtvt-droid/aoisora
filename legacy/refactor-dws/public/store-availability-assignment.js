...
let shiftCodes = []
let weeklyWorkload = new Map() // Map<employeeId, shiftCount>
...
const mockStaffProfiles = [
  // Profile 1: 6 ngày (2 ca), 3 ngày (1 ca), 1 ngày (0 ca)
  {
    '2025-11-26': {
      registrations: [
        { shiftCode: 'V812', priority: 1 },
        { shiftCode: 'V829', priority: 1 },
      ],
    },
    '2025-11-27': {
      registrations: [
...
async function loadWorkPositions() {
  try {
    ...
    workPositions = querySnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
...
async function fetchInitialData() {
  ...
  allEmployeesInStore = employeeSnapshot.docs.map(doc => ({
    id: doc.id,
    ...doc.data(),
  }))
...
function renderAssignmentTable() {
  ...
  const positionHeaders = workPositions
    .map(
      pos => `
  ...
  const sortedEmployees = [...allEmployeesInStore].sort((a, b) => {
    ...
    if (priorityA === 4) {
      // Cả hai đều là STAFF
      const hasAvailabilityA = allAvailabilities.some(
        avail =>
          avail.employeeId === a.id &&
          avail.registrations.some(reg => reg.shiftCode)
      )
      const hasAvailabilityB = allAvailabilities.some(
        avail =>
          avail.employeeId === b.id &&
          avail.registrations.some(reg => reg.shiftCode)
      )
...
function renderEditableCellForLeader(employee, date) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const leaderShiftOptions = shiftCodes
      ...
      .map(
        sc =>
          `<option value="${sc.shiftCode}" ${reg.shiftCode === sc.shiftCode ? 'selected' : ''}>${sc.shiftCode}</option>`
      )
      .join('')
...
function renderReadOnlyCellForStaff(employee, date) {
  ...
  const timeRange1 =
    shiftCodes.find(sc => sc.shiftCode === reg1.shiftCode)?.timeRange ||
    '&nbsp;'
  const timeRange2 =
    shiftCodes.find(sc => sc.shiftCode === reg2.shiftCode)?.timeRange ||
    '&nbsp;'
  const priorityIcon1 =
    reg1.priority === 1
      ? '<i class="fas fa-circle text-green-500" title="Chắc chắn"></i>'
      : reg1.priority === 2
        ? '<i class="fas fa-triangle-exclamation text-amber-500" title="Có thể"></i>'
        : '<i class="fas fa-circle text-transparent"></i>'
  ...
  const workPositionOptions = filteredWorkPositions
...
async function handleSaveAssignmentForDay(event) {
  ...
  const employeeRows = document.querySelectorAll(
    '#assignment-table-body tr[data-employee-id]'
  )
  ...
  for (const row of employeeRows) {
    ...
    if (
      ...
      (positionInputs.length > 0 && positionInputs[0].disabled)
    ) {
      continue
    }
    ...
    for (let i = 0; i < 2; i++) {
      const reg = registrations[i] || {}
      const input = Array.from(positionInputs).find(
        inp => inp.dataset.shiftIndex == i
      )
      ...
      if (reg.shiftCode) {
        if (!positionId) {
          ...
          window.showToast(
            `Vui lòng phân công vị trí cho ${employee.name} (Ca ${i + 1})`,
            'warning'
          )
        } else {
  ...
  try {
    ...
    const existingSchedules = scheduleSnapshot.docs.map(d => ({
      id: d.id,
      ...d.data(),
    }))
    ...
    for (const empAssignment of assignments) {
      for (const as of empAssignment.assignments) {
        ...
        const scheduleDoc = existingSchedules.find(
          s =>
            s.employeeId === empAssignment.employeeId &&
            s.shift === as.shiftCode
        )
...
/**
 * Tính toán số lượng ca làm việc đã đăng ký trong tuần cho mỗi nhân viên.
 * Kết quả được lưu vào biến `weeklyWorkload`.
 */
function calculateWeeklyWorkload() {
  weeklyWorkload.clear() // Xóa dữ liệu cũ

  // Khởi tạo số ca cho tất cả nhân viên là 0
  allEmployeesInStore.forEach(emp => {
    weeklyWorkload.set(emp.id, 0)
  })

  // Duyệt qua tất cả các đăng ký trong tuần và đếm
  allAvailabilities.forEach(avail => {
    if (avail.registrations) {
      const shiftCount = avail.registrations.filter(reg => reg.shiftCode).length
      if (shiftCount > 0) {
        weeklyWorkload.set(
          avail.employeeId,
          (weeklyWorkload.get(avail.employeeId) || 0) + shiftCount
        )
      }
    }
  })
...
async function handleSuggestAssignmentForDay(event) {
  ...
  // Tính toán lại khối lượng công việc của tuần trước khi đề xuất
  ...
  if (totalAvailableShifts === 0) {
    window.showToast(
      'Không có nhân viên nào đăng ký ca trong ngày này.',
      'info'
    )
    return
  }
  ...
  const remainders = percentages.map(
    (p, i) => totalAvailableShifts * p - counts[i]
  )
  while (remainder > 0) {
    let maxIdx = -1
    let maxRem = -1
  ...
  counts.forEach((count, index) => {
    for (let i = 0; i < count; i++)
      neededPositions.push(workPositions[index].name)
  })
  ...
  for (const slot of availableStaffSlots) {
    ...
    const existingAssignment = suggestions.find(
      s => s.employeeId === slot.employeeId && s.shiftIndex === slot.shiftIndex
    )
  ...
  document
    .querySelectorAll(`td[data-date="${date}"] .position-input`)
    .forEach(input => {
      input.value = ''
    })
  ...
  suggestions.forEach(suggestion => {
    const input = document.querySelector(
      `tr[data-employee-id="${suggestion.employeeId}"] td[data-date="${date}"] .position-input[data-shift-index="${suggestion.shiftIndex}"]`
    )
...
function handleLeaderCellInteraction(event) {
  ...
  if (priorityBtn) {
    ...
    if (!icon.classList.contains('text-gray-300')) {
      ...
      // Nếu chưa được chọn, chọn nó và bỏ chọn cái còn lại
      block.querySelectorAll('.priority-btn').forEach(btn => {
        btn
          .querySelector('i')
          .classList.remove('text-green-500', 'text-amber-500')
...
async function saveLeaderAvailabilityForDate(date) {
  ...
  const td = document.querySelector(
    `td[data-date="${date}"][data-is-editable="true"]`
  )
  ...
  td.querySelectorAll('.shift-registration-block').forEach(block => {
    ...
    const selectedPriorityBtn = block.querySelector(
      '.priority-btn i:not(.text-gray-300)'
    )
    const priority = selectedPriorityBtn
      ? parseInt(
          selectedPriorityBtn.closest('.priority-btn').dataset.priority,
          10
        )
      : 0
      ...
      if (schedules) {
        schedules.forEach((schedule, index) => {
          const input = row.querySelector(
            `td[data-date="${date}"] .position-input[data-shift-index="${index}"]`
          )
...
function updateEmployeeStats() {
  ...
  employeeRows.forEach(row => {
    ...
    positionSelects.forEach(select => {
      ...
      currentHoursMap.set(
        positionName,
        (currentHoursMap.get(positionName) || 0) + shiftInfo.duration
      )
    })
  ...
  employeeRows.forEach(row => {
    ...
    const totalAssignedHours = positionHours
      ? Array.from(positionHours.values()).reduce(
          (sum, hours) => sum + hours,
          0
        )
      : 0
    ...
    workPositions.forEach(pos => {
      const statsCell = row.querySelector(
        `div[data-position-name="${pos.name}"]`
      )
      if (!statsCell) return

      // Lấy giờ làm cho vị trí, nếu không có thì là 0
      const hoursForPosition = positionHours
        ? positionHours.get(pos.name) || 0
        : 0
      const percentage =
        totalAssignedHours > 0
          ? (hoursForPosition / totalAssignedHours) * 100
          : 0
      ...
      percentageSpan.classList.remove(
        'text-red-600',
        'text-blue-600',
        'text-green-600',
        'text-gray-700'
