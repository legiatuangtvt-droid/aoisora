...
async function fetchInitialData() {
  ...
  allEmployeeStatuses = statusesSnapshot.docs.map(doc => ({
    id: doc.id,
    ...doc.data(),
  }))
}
...
function renderEmployeeList(employeeList) {
  ...
  employeeList.forEach((employee, index) => {
    ...
    const roleInfo = allRoles.find(r => r.id === employee.roleId) || {
      name: employee.roleId || 'N/A',
    }
    ...
    switch (employee.roleId) {
      case 'AREA_MANAGER': {
        const managedArea = allAreas.find(a =>
          employee.managedAreaIds?.includes(a.id)
        )
        storeInfoText = managedArea
          ? managedArea.name
          : 'Khu vực không xác định'
        const managedStoresInArea = allStores.filter(
          s => s.areaId === managedArea?.id
        )
        storeInfoTitle =
          managedStoresInArea.length > 0
      ...
      case 'STORE_INCHARGE': {
        ...
        storeInfoText = managedStore
          ? managedStore.name
          : employee.storeId || 'N/A'
        storeInfoTitle = `Quản lý: ${allStores
          .filter(s => employee.managedStoreIds?.includes(s.id))
          .map(s => s.name)
          .join(', ')}`
        break
      }
      case 'REGIONAL_MANAGER': {
        const managedRegion = allRegions.find(
          r => r.id === employee.managedRegionId
        )
        storeInfoText = managedRegion
          ? managedRegion.name
          : 'Miền không xác định'
        const managedAreasInRegion = allAreas.filter(
          a => a.regionId === managedRegion?.id
        )
        storeInfoTitle =
          managedAreasInRegion.length > 0
    ...
    const statusInfo = allEmployeeStatuses.find(
      s => s.id === employee.status
    ) || { name: employee.status, color: 'gray' }
...
function openEmployeeModal(employeeId = null) {
  ...
  roleSelect.innerHTML = allRoles
    ...
    .join('')
  storeSelect.innerHTML = allStores
    .map(s => `<option value="${s.id}">${s.name}</option>`)
    .join('')
  statusSelect.innerHTML = allEmployeeStatuses
    .map(s => `<option value="${s.id}">${s.name}</option>`)
    .join('')
  ...
  storeSelect.insertAdjacentHTML(
    'afterbegin',
  ...
  if (employeeId) {
    ...
    modalTitle.innerHTML =
      '<i class="fas fa-user-edit mr-2 text-indigo-500"></i> Chỉnh Sửa Nhân Viên'
    submitBtn.innerHTML = '<i class="fas fa-save mr-1"></i> Cập Nhật'
    idInput.value = employeeId
    idInput.readOnly = true

    // Lấy thông tin nhân viên từ mảng dữ liệu đã tải
    ...
    if (employee) {
      ...
      showToast(
        `Không tìm thấy thông tin cho nhân viên ID: ${employeeId}`,
        'error'
      )
      return // Không mở modal nếu không có dữ liệu
    }
  } else {
    // Chế độ Thêm
    modalTitle.innerHTML =
      '<i class="fas fa-user-plus mr-2 text-indigo-500"></i> Thêm Nhân Viên Mới'
...
async function fetchAndRenderEmployees() {
  ...
  const [employeeSnap, areaManagerSnap, regionalManagerSnap] =
    await Promise.all([employeeQuery, areaManagerQuery, regionalManagerQuery])
  ...
  if (
    currentUser &&
    ['STORE_INCHARGE', 'AREA_MANAGER', 'REGIONAL_MANAGER'].includes(
      currentUser.roleId
    )
  ) {
    let managedStoreIds = []
  ...
  personnel.sort((a, b) => {
    for (const criterion of sortCriteria) {
      ...
      const valA = a[column] || ''
      const valB = b[column] || ''
      const comparison = String(valA).localeCompare(String(valB), 'vi', {
        sensitivity: 'base',
      })
      if (comparison !== 0) {
  ...
  allPersonnel = personnel // Cập nhật lại mảng toàn cục sau khi lọc và sắp xếp
  totalEmployees = allPersonnel.length

  // 3. Áp dụng Pagination
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const employeesForPage = personnel.slice(startIndex, endIndex)
  ...
  try {
    ...
    window.showToast(
      'Lỗi khi tải dữ liệu. Firestore index có thể bị thiếu.',
      'error'
    )
    if (listElement) {
  ...
  if (isFilterable) {
    ...
    const renderFilterItems = (searchTerm = '') => {
      ...
      filteredOptions.forEach(opt => {
        ...
        item.href = '#'
        item.className =
          'filter-item block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100'
...
export async function init() {
    ...
    employeeForm.addEventListener(
      ...
      async e => {
        ...
        try {
          ...
          showToast(
            'Đã xảy ra lỗi khi lưu. Mã nhân viên có thể đã tồn tại.',
            'error'
          )
        }
      },
  ...
  document
    .getElementById('managed-roles-list')
    ?.closest('table')
    .addEventListener(
      'click',
      e => {
        ...
        showConfirmation(
          `Bạn có chắc chắn muốn xóa chức vụ "${roleName}"?`
  ...
  document.getElementById('employee-list')?.addEventListener(
    'click',
    e => {
  ...
  document.getElementById('pagination-controls')?.addEventListener(
    'click',
    e => {
