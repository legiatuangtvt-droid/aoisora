...
function createStoreFilter() {
  ...
  const adminSettingsContainer = document.getElementById(
  ...
  if (currentUser?.roleId === 'ADMIN' && adminSettingsContainer) {
    ...
    saveBtn?.addEventListener('click', async () => {
      ...
      if (newDay >= 1 && newDay <= 28) {
        ...
        await setDoc(payrollSettingsRef, {
          startDay: newDay,
  ...
  if (currentUser.level >= 20) {
    ...
    document.getElementById('schedule-container').innerHTML =
      '<div class="text-center p-10 text-gray-500">Không thể xác định cửa hàng của bạn.</div>'
  }
}
...
async function loadScheduleForStore(storeId) {
  ...
  try {
    ...
    document.getElementById('schedule-container').innerHTML =
      '<div class="text-center p-10 text-red-500">Đã xảy ra lỗi khi tải dữ liệu.</div>'
  }
}
...
function renderCalendar(schedules, employees) {
  ...
  for (
    ...
    if (currentUser && currentUser.roleId === 'STAFF') {
      ...
      const userSchedule = schedulesForDay.find(
        s => s.employeeId === currentUser.id
      )
      if (userSchedule) {
        const store = allStores.find(st => st.id === userSchedule.storeId)
        const shiftInfo = allShiftCodes.find(
          sc => sc.shiftCode === userSchedule.shift
        )
    ...
    let cellClasses =
      'relative p-2 border-t border-l border-gray-200 flex flex-col [&:nth-child(7n)]:border-r h-32'
...
export async function init() {
  ...
  // Trả về controller để module cha có thể gọi cleanup
