...
import { calculateREForTask, calculateREForGroup } from './re-calculator.js'
...
export function generateSchedule(
  ...
  taskSlotsToPlace.sort((a, b) => {
    ...
    return (
      ...
      b.originalNumSlots - a.originalNumSlots
    )
  })
  ...
  for (let i = 0; i < shiftCodeOrder.length; i++) {
    ...
    const shiftInfo = allShiftCodes.find(
      sc => sc.shiftCode === shiftCodeForThisRow
    )
    ...
    for (
      let currentMinute = shiftStartMinutes;
      currentMinute < shiftEndMinutes;
      currentMinute += 15
    ) {
      const hour = Math.floor(currentMinute / 60)
      const minute = currentMinute % 60
      shiftData.availableSlots.push({
        shiftId,
  ...
  if (placementTasks.length > 0) {
    ...
    const placementHandlers = {
      firstOfDay: () => {
        const tasks = placementTasks.filter(
          t => t.shiftPlacement.type === 'firstOfDay'
        )
        tasks.forEach(task => {
      ...
      breakTime: () => {
        ...
        const scheduleBreakForShift = (shift, mealType) => {
          ...
          for (
            let startMinute = searchStart;
            startMinute <= searchEnd - 60;
            startMinute += 15
          ) {
            const slotsNeeded = []
            let canPlace = true
            for (let i = 0; i < 4; i++) {
              ...
              if (
                newScheduleData[shift.shiftId].some(
            ...
            if (canPlace) {
              ...
              slotsNeeded.forEach(startTime =>
                placeTaskInSlot(shift, startTime, breakTask)
              )
              break
            }
          }
      ...
      lastOfDay: () => {
        const tasks = placementTasks.filter(
          t => t.shiftPlacement.type === 'lastOfDay'
        )
        tasks.forEach(task => {
          ;(task.allowedPositions || []).forEach(posName => {
            const shiftsForPos = positionShifts[posName]
      ...
      firstOfShift: () => {
        const tasks = placementTasks.filter(
          t => t.shiftPlacement.type === 'firstOfShift'
        )
        plannedShifts.forEach(shift => {
          const shiftPositionName = positionIdToNameMap[shift.positionId]
          const appropriateTask = tasks.find(t =>
            (t.allowedPositions || []).includes(shiftPositionName)
          )
          if (!appropriateTask) return

          const firstShiftOfDayForPosition =
            positionShifts[shiftPositionName]?.[0]
          if (firstShiftOfDayForPosition?.shiftId === shift.shiftId) {
            ...
            if (
              newScheduleData[shift.shiftId].some(
                t => t.startTime === firstSlotStartTime
              )
            )
              return
          }
          ...
          for (
            let minute = shift.startMinutes;
            minute < shift.endMinutes;
            minute += 15
          ) {
      ...
      lastOfShift: () => {
        plannedShifts.forEach(shift => {
          const shiftPositionName = positionIdToNameMap[shift.positionId]
          const appropriateTask = placementTasks.find(
            t =>
          ...
          const lastShiftOfDayForPosition =
            positionShifts[shiftPositionName]?.[
              positionShifts[shiftPositionName].length - 1
            ]
          if (lastShiftOfDayForPosition?.shiftId === shift.shiftId) {
            ...
            if (
              newScheduleData[shift.shiftId].some(
                t => t.startTime === lastSlotStartTime
              )
            )
              return
          }
          ...
          for (
            let minute = shift.endMinutes - 15;
    ...
    handlerOrder.forEach(handlerType => {
      const hasTaskForHandler = placementTasks.some(
        t => t.shiftPlacement.type === handlerType
      )
    ...
    for (let hour = openHour; hour < closeHour; hour++) {
      ...
      for (let quarter = 0; quarter < 60; quarter += 15) {
        ...
        const findAndPlacePosTask = (taskName, preferredPositions) => {
          ...
          for (const posName of preferredPositions) {
            const availableShift = plannedShifts.find(shift => {
              ...
              const isWithinShift =
                timeToMinutes(startTime) >= shift.startMinutes &&
                timeToMinutes(startTime) < shift.endMinutes
              const isSlotFree = !newScheduleData[shift.shiftId].some(
                t => t.startTime === startTime
              )
        ...
        if (findAndPlacePosTask('POS 1', ['POS', 'Leader'])) {
          ...
          if (requiredPosSlots > 0)
            findAndPlacePosTask('POS 2', ['POS', 'Leader'])
          if (requiredPosSlots > 0)
            findAndPlacePosTask('POS 3', ['POS', 'Leader'])
          // Nếu vẫn còn thiếu, sử dụng "Hỗ trợ POS" làm phương án dự phòng
          if (requiredPosSlots > 0)
            findAndPlacePosTask('Hỗ trợ POS', [
              'POS',
              'Leader',
              'Ngành hàng',
              'Aeon Cafe',
  ...
  while (slotsFilledInLastPass !== 0) {
    ...
    for (const taskToAssign of taskSlotsToPlace) {
      ...
      const taskInfo = taskGroupsArray
        .find(g => g.id === taskToAssign.groupId)
        ?.tasks.find(t => t.name === taskToAssign.taskName)
      if (!taskInfo) continue

      // --- LOGIC MỚI: Ưu tiên theo thứ tự trong allowedPositions ---
      const preferredPositions =
        taskInfo.allowedPositions && taskInfo.allowedPositions.length > 0
      ...
      for (const positionName of preferredPositions) {
        if (
          taskToAssign.numSlotsRemaining <= 0 ||
          scheduledManHours >= flexibleTargetManHours
        )
          break

        // Lấy tất cả các slot trống thuộc về vị trí đang xét và xáo trộn chúng
        const slotsForPosition = getAllAvailableSlots().filter(
          slot => positionIdToNameMap[slot.shift.positionId] === positionName
        )
        ...
        for (const slot of slotsForPosition) {
          ...
          if (
            newScheduleData[slot.shift.shiftId].some(
          ...
          const isSlotInValidTimeWindow =
            !taskInfo.timeWindows ||
            taskInfo.timeWindows.length === 0 ||
            taskInfo.timeWindows.some(window => {
          ...
          if (isSlotInValidTimeWindow) {
            ...
            if (limit === 0 || limit === undefined || currentCount < limit) {
              newScheduleData[slot.shift.shiftId].push({
  ...
  for (const shiftId in newScheduleData) {
    newScheduleData[shiftId].sort((a, b) =>
