...
function filterDatalist(datalistElement, selfShiftCode, otherShiftCode) {
  ...
  const otherShift = otherShiftCode
    ? localShiftCodes.find(sc => sc.shiftCode === otherShiftCode)
    : null
  let startOther = null,
    endOther = null
  if (otherShift) {
    const [startStr, endStr] = otherShift.timeRange
      .split('~')
      .map(s => s.trim())
  ...
  localShiftCodes.forEach(potentialShift => {
    ...
    if (!otherShift || !otherShiftCode) {
      ...
      const [startPotentialStr, endPotentialStr] = potentialShift.timeRange
        .split('~')
        .map(s => s.trim())
...
function loadAvailabilityForDesktopView(availabilityData) {
  const allCells = document.querySelectorAll(
    '#availability-table-body td[data-date]'
  )
  allCells.forEach(td => {
...
function handleCellFocus(event) {
  ...
  const otherDatalist = otherInput
    ? document.getElementById(otherInput.getAttribute('list'))
    : null
  ...
  if (targetDatalist)
    filterDatalist(
      targetDatalist,
      target.value,
      otherInput ? otherInput.value : null
    )
  if (otherDatalist)
    filterDatalist(otherDatalist, otherInput.value, target.value)
}
...
async function saveWeekAvailability() {
  ...
  allCells.forEach(td => {
    ...
    td.querySelectorAll('.shift-registration-block').forEach(block => {
      ...
      const priority = shiftCode
        ? parseInt(priorityBtn.dataset.priority, 10)
        : 0
  ...
  try {
    ...
    for (const date in dataByDate) {
      ...
      const hasRegistration = dataByDate[date]?.some(
        reg => reg && reg.shiftCode
      )
      ...
      if (hasRegistration || dataByDate[date].helpShiftCode) {
        const dataToSet = {
          ...
          date: date,
          registrations: dataByDate[date].filter(
...
function openHelpModal(date) {
  ...
  document.getElementById('help-modal-title').textContent =
    `Đăng ký ca hỗ trợ ngày ${dateObj.toLocaleDateString('vi-VN')}`
...
function updateHelpButtonUI(button, helpShiftCode) {
  ...
  if (hasHelpShift) {
    ...
    button.title = shiftInfo
      ? `Hỗ trợ ca ${helpShiftCode} (${shiftInfo.timeRange})`
      : 'Đăng ký ca hỗ trợ'
...
function handleSaveHelpTime() {
  ...
  const mainShiftInputs = document.querySelectorAll(
    `td[data-date="${date}"] .shift-input`
  )
  for (const input of mainShiftInputs) {
    if (input.value) {
      const shift = localShiftCodes.find(sc => sc.shiftCode === input.value)
      if (shift) {
        const [shiftStartStr, shiftEndStr] = shift.timeRange
          .split('~')
          .map(s => s.trim())
        ...
        if (selectedShiftCode) {
          const helpShift = localShiftCodes.find(
            sc => sc.shiftCode === selectedShiftCode
          )
          if (helpShift) {
            const [helpStartStr, helpEndStr] = helpShift.timeRange
              .split('~')
              .map(s => s.trim())
            const helpStartMinutes = timeToMinutes(helpStartStr)
            const helpEndMinutes = timeToMinutes(helpEndStr)
            if (
              helpStartMinutes < shiftEndMinutes &&
              shiftStartMinutes < helpEndMinutes
            ) {
              window.showToast(
                `Lỗi: Ca hỗ trợ (${helpShift.timeRange}) bị trùng với ca làm việc chính (${shift.timeRange}).`,
                'error'
              )
              return
            }
...
function addEventListeners() {
  ...
  document
    .getElementById('save-week-availability-btn')
    ?.addEventListener('click', saveWeekAvailability, { signal })
  document
    .getElementById('save-help-time-btn')
    ?.addEventListener('click', handleSaveHelpTime, { signal })
...
export async function renderDesktopView({
  payrollCycle,
  shiftCodes,
  availabilityData,
  formatDate,
  ...
  const allDates = []
  for (
    let d = new Date(payrollCycle.start);
    d <= payrollCycle.end;
  ...
  for (let i = 0; i < allDates.length; i += 7) {
    ...
    weekDates.forEach(date => {
      ...
      headerCell.querySelector('.day-name').textContent =
        date.toLocaleDateString('en-US', { weekday: 'short' })
      headerCell.querySelector('.date-number').textContent =
