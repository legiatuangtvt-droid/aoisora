...
async function listenForTaskChanges() {
  ...
  const taskGroups = taskGroupsSnapshot.docs.map(doc => ({
    id: doc.id,
    ...doc.data(),
  }))
...
function renderPaginationControls() {
  ...
  paginationContainer
    .querySelectorAll('.pagination-btn[data-page="prev"]')
    .forEach(btn => (btn.disabled = isFirstPage))
  paginationContainer
    .querySelectorAll('.pagination-btn[data-page="next"]')
    .forEach(btn => (btn.disabled = isLastPage))
}
...
function filterAndRender() {
  const searchTerm =
    document.getElementById('search-task-input')?.value.toLowerCase() || ''
...
function openTaskModal(task = null) {
  ...
  if (!modal || !modalTitle || !form || !submitButton) {
    console.error(
      'One or more essential modal elements are missing from the DOM.'
    )
    window.showToast(
      'Lỗi: Không thể mở form. Giao diện người dùng bị thiếu.',
      'error'
    )
    return
  }
  ...
  if (task) {
    ...
    document.getElementById('task-frequency-number').value =
      task.frequencyNumber || ''
    document.getElementById('task-re-unit').value = task.reUnit || ''
    document.getElementById('task-manual-number').value =
      task.manual_number || ''
...
async function handleFormSubmit(e) {
  ...
  try {
    if (isEditMode) {
      ...
      if (taskIndex === -1) {
        throw new Error(
          `Không tìm thấy task với order ${originalOrder} trong nhóm.`
        )
      }
      ...
      tasks[taskIndex].frequency =
        document.getElementById('task-frequency').value
      tasks[taskIndex].frequencyNumber = document.getElementById(
        'task-frequency-number'
      ).value
      tasks[taskIndex].reUnit = document.getElementById('task-re-unit').value
      tasks[taskIndex].manual_number =
        document.getElementById('task-manual-number').value
      tasks[taskIndex].manualLink =
        document.getElementById('task-manual-link').value
      ...
      let tasks = docSnap.data().tasks || []
      const newOrder =
        tasks.length > 0 ? Math.max(...tasks.map(t => t.order)) + 1 : 1
    ...
    if (isEditMode) {
      ...
      submitButton.innerHTML =
        '<i class="fas fa-plus-circle mr-2"></i>Thêm Task'
    }
  }
}
...
function exportTasksToExcel() {
  if (typeof XLSX === 'undefined') {
    window.showToast(
      'Lỗi: Thư viện Excel chưa được tải. Vui lòng thử lại.',
      'error'
    )
    return
  }
  ...
  XLSX.writeFile(
    workbook,
    `RE_Tasks_Export_${new Date().toISOString().slice(0, 10)}.xlsx`
  )
}
    ...
    reader.onload = async e => {
      try {
        ...
        for (const groupCode in tasksByGroup) {
          // Sửa lỗi no-prototype-builtins: Gọi hasOwnProperty một cách an toàn.
          if (!Object.prototype.hasOwnProperty.call(tasksByGroup, groupCode)) {
            continue
          }
          ...
          if (importMode === 'append') {
            ...
            const existingTasks = docSnap.exists()
              ? docSnap.data().tasks || []
              : []
            const lastOrder = existingTasks.reduce(
              (max, task) => Math.max(max, task.order || 0),
              0
            )

            const newTasksWithOrder = tasksByGroup[groupCode].map(
              (task, index) => ({ ...task, order: lastOrder + 1 + index })
            )
        ...
        await batch.commit()
        window.showToast(
          `Import thành công! Đã cập nhật ${jsonData.length} tasks.`,
          'success'
        )
        ...
        window.showToast(
          error.message || 'Đã xảy ra lỗi khi đọc file.',
          'error'
        )
      }
    }
...
function downloadTemplateFile() {
  ...
  const exampleData = [
    [
      'POS',
      'Product',
      'Quét sàn khu vực POS',
      'Daily',
      '1',
      '5',
      'MN-01',
      'http://example.com/manual/mn01',
      'Quét sạch sẽ trước giờ mở cửa',
    ],
    [
      'DRY',
      'Fixed',
      ...
      '15',
      'MN-02',
      'http://example.com/manual/mn02',
      'Thực hiện vào Thứ 3 và Thứ 6',
    ],
  ]
...
export function init() {
  ...
  document
    .getElementById('re-task-form')
    ?.addEventListener('submit', handleFormSubmit, { signal })
  document.getElementById('export-tasks-btn')?.addEventListener(
    'click',
    event => {
  ...
  document.getElementById('import-form')?.addEventListener(
    'submit',
    e => {
      e.preventDefault()
  ...
  document
    .getElementById('search-task-input')
    ?.addEventListener('input', filterAndRender, { signal })

  // Gắn sự kiện cho phân trang
  document.getElementById('pagination-container')?.addEventListener(
    'click',
    e => {
