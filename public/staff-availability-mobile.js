 ...
 */
function renderMobileCalendar({ payrollCycle, availabilityData, formatDate }) {
  ...
  for (let i = 0; i < daysInCycle; i++) {
    ...
    if (d.getDate() === 1 || new Date(d.getTime() + 86400000).getDate() === 1)
...
function renderShiftLegend({ shiftCodes, availabilityData, selectedDateStr }) {
  ...
  if (selectedDateStr && availabilityData[selectedDateStr]) {
    ...
    if (schedule && schedule.shift) {
      const assignedShift = shiftCodes.find(
  ...
  if (shiftsToShow.length === 0) {
    legendBody.innerHTML =
      '<div class="text-gray-500 italic">Không có ca làm việc nào được phân công cho ngày này.</div>'
  } else {
    // Tạo một dòng kẻ liền mạch bằng cách sử dụng col-span-2
    legendBody.innerHTML = shiftsToShow
      .map(
        (sc, index) => `
...
function handleDaySelection(dateStr = null, config) {
  ...
  const targetCell = grid.querySelector(
    `.calendar-day[data-date='${targetDateStr}']`
  )
