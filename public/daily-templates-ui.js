...
export function toggleTemplateBuilderLock(locked) {
  ...
  gridContainer
    .querySelectorAll('.shift-code-selector, .work-position-selector')
    .forEach(el => {
      el.disabled = locked
    })
  ...
  gridContainer
    .querySelectorAll('.delete-shift-row-btn')
...
export function createShiftCodeDatalist() {
  ...
  datalist.innerHTML = allShiftCodes
    .map(sc => `<option value="${sc.shiftCode}">${sc.timeRange}</option>`)
    .join('')
...
export function updateGridHeaderStats() {
  ...
  document
    .querySelectorAll('#template-builder-tbody tr[data-shift-id]')
    .forEach(row => {
  ...
  thead.querySelectorAll('th[data-hour]').forEach(headerCell => {
    ...
    const { positionCount, posManhour, posManhourColor } =
      calculateHourlyStatsForTime(
        time,
        currentShiftMappings,
        currentSchedule,
        reParameters,
        posGroupId
      )
    ...
    const posManhourContainerEl = headerCell.querySelector(
      '.pos-manhour-container'
    ) // Lấy container để đổi màu

    if (positionCountEl) positionCountEl.textContent = positionCount
    if (posManhourContainerEl)
      posManhourContainerEl.className = `pos-manhour-container ${posManhourColor}` // Cập nhật class màu
    if (posManhourContainerEl)
      posManhourContainerEl.querySelector('.hourly-pos-manhour').textContent =
        posManhour
  })
}
...
export function renderGrid(templateData = null) {
  ...
  table.className =
    'min-w-full border-collapse border border-slate-200 table-fixed'
  ...
  timeSlots.forEach(time => {
    const { positionCount, posManhour, posManhourColor } =
      calculateHourlyStatsForTime(time, shiftMappings, schedule, reParameters)
    headerRowHtml += `
  ...
  if (templateData && shiftMappings) {
    const sortedShiftIds = Object.keys(shiftMappings).sort((a, b) => {
      ...
      const getStartTime = shift => {
        const shiftInfo = allShiftCodes.find(
          sc => sc.shiftCode === shift.shiftCode
        )
        return shiftInfo
          ? timeToMinutes(shiftInfo.timeRange.split('~')[0])
          : 9999
      }
    ...
    sortedShiftIds.forEach(shiftId => {
      ...
      const shiftInfo = allShiftCodes.find(
        sc => sc.shiftCode === mapping.shiftCode
      )
      ...
      const workPositionOptions = allWorkPositions
        ? allWorkPositions
            .map(
              pos =>
                `<option value="${pos.id}" ${pos.id === mapping.positionId ? 'selected' : ''}>${pos.name}</option>`
            )
            .join('')
        : ''
      const deleteButtonHTML = isManager
        ? ''
        : `<button class="delete-shift-row-btn absolute top-1 right-1 text-gray-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity" title="Xóa dòng ca này"><i class="fas fa-times"></i></button>`
      let firstCellHtml = `
 ...
 */
function calculateHourlyStatsForTime(
  time,
  shiftMappings,
  schedule,
  reParameters = {},
  ...
  for (const shiftId in schedule) {
    if (schedule[shiftId]) {
      schedule[shiftId].forEach(task => {
        ...
        if (
          taskStartMinutes >= hourStartMinutes &&
          taskStartMinutes < hourEndMinutes
        ) {
          totalTasksInHour++
  ...
  return {
    positionCount: totalManhour,
    posManhour: standardPosManhour.toFixed(2),
    posManhourColor: posManhourColor,
  }
}
...
export async function renderPlanTracker(plan) {
  // Giữ lại async vì có các thao tác bất đồng bộ bên trong
  ...
  if (plan.history && plan.history.length > 0) {
    const historyHTML = plan.history
      ...
      .map(async historyEntry => {
        ...
        const timestamp =
          historyEntry.timestamp?.toDate().toLocaleString('vi-VN') || 'N/A'
        const iconClass = historyEntry.status.includes('REJECTED')
        ...
        if (historyEntry.status === 'HQ_APPLIED' && plan.deploymentBatchId) {
          ...
          const relatedPlansQuery = query(
            collection(db, 'monthly_plans'),
            where('deploymentBatchId', '==', plan.deploymentBatchId)
          )
          ...
          const allRmsDeployed =
            relatedPlans.length > 0 &&
            relatedPlans.every(p =>
          ...
          const rmStatusesPromises = relatedPlans.map(async p => {
            const regionalManager = allPersonnel.find(
              person =>
                person.roleId === 'REGIONAL_MANAGER' &&
                person.managedRegionId === p.regionId
            )
            ...
            const sentToStaffStep = p.history.find(
              h => h.status === 'RM_SENT_TO_STAFF'
            )

            if (sentToStaffStep) {
              // Nếu RM đã triển khai cho nhân viên, hiển thị thông báo "Đã triển khai"
              const sentTimestamp =
                sentToStaffStep.timestamp?.toDate().toLocaleString('vi-VN') ||
                ''
              ...
              const nextStepLabel =
                steps
                  .find(s => s.id === nextStep.status)
                  ?.label.replace(/^\d+\.\d*\.?\s*/, '') || nextStep.status // Bỏ số thứ tự đầu dòng
              const nextStepTimestamp =
                nextStep.timestamp?.toDate().toLocaleString('vi-VN') || ''
...
export async function toggleBuilderView() {
  const builderContainer = document.getElementById(
    'template-builder-grid-container'
  )
  ...
  if (currentView === 'builder') {
    ...
    if (
      window.currentUser &&
      (window.currentUser.roleId === 'HQ_STAFF' ||
        window.currentUser.roleId === 'ADMIN')
    ) {
