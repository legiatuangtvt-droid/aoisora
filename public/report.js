...
// DOM Elements
let leaderboardListEmployee,
  leaderboardListStore,
  initialPrompt,
  employeeDetailView,
  storeDetailView
...
async function fetchPerformanceData() {
  try {
    ...
    const [employeesSnap, rolesSnap, taskGroupsSnap, storesSnap] =
      await Promise.all([
    ...
    allEmployees.sort(
      (a, b) => (b.experiencePoints || 0) - (a.experiencePoints || 0)
    )

    return allEmployees
  } catch (error) {
    console.error('Lỗi khi tải dữ liệu hiệu suất:', error)
    if (leaderboardListEmployee) {
      leaderboardListEmployee.innerHTML = `<p class="p-4 text-red-500">Không thể tải dữ liệu nhân viên.</p>`
    }
    return []
  }
}
...
function renderLeaderboard(employees) {
  ...
  employees.forEach((employee, index) => {
    ...
    employeeCard.addEventListener('click', () => {
      ...
      document
        .querySelectorAll('#leaderboard-list-employee > div')
        .forEach(child => child.classList.remove('bg-indigo-100'))
...
async function drawSkillRadarChart(employeeId) {
  ...
  const ctx = document
    .getElementById('skill-radar-chart-canvas')
    ?.getContext('2d')
  if (!ctx) return
...
function displayEmployeeDetails(employeeId) {
  ...
  const xpProgress =
    ((currentXP - levelInfo.currentLevelXP) /
      (levelInfo.xpForNextLevel - levelInfo.currentLevelXP)) *
    100
  const mockData = getMockDetails(employeeId)
  ...
  document.getElementById('employee-role').textContent =
    allRoles[employee.roleId]?.name || 'Chưa có chức vụ'
  document.querySelector('#employee-detail-view .level-badge').textContent =
    `Lv.${levelInfo.level}`
  document.querySelector('#employee-detail-view .xp-bar').style.width =
    `${xpProgress}%`
  document.querySelector(
    '#employee-detail-view .text-xs.text-right'
  ).textContent =
    `${currentXP.toLocaleString()} / ${levelInfo.xpForNextLevel.toLocaleString()} XP để lên cấp`
  ...
  const performanceStatsContainer = document.querySelector(
    '#employee-detail-view .p-4.border.rounded-lg .grid'
  )
  const trendIcon =
    mockData.performance.trend === 'improve'
      ? 'fa-arrow-up text-green-500'
      : 'fa-arrow-down text-red-500'
  const trendText =
    mockData.performance.trend === 'improve' ? 'Cải thiện' : 'Giảm sút'
  ...
  const recommendationContainer = document.querySelector(
    '#employee-detail-view .bg-indigo-50 .text-sm.space-y-3'
  )
...
async function drawXpCompositionChart(employeeId) {
  const mockData = getMockDetails(employeeId)
  const ctx = document
    .getElementById('xp-composition-chart-canvas')
    ?.getContext('2d')
  if (!ctx) return
...
function displayStoreDetails(storeId) {
  ...
  document.getElementById('store-detail-xp').querySelector('span').textContent =
  ...
  const employeeListContainer = document.getElementById(
    'store-employee-list-container'
  )
  employeeListContainer.innerHTML =
    '<h4 class="font-semibold mb-2">Xếp hạng nhân viên trong cửa hàng</h4>'
  if (storeEmployees.length > 0) {
    ...
    employeeListContainer.innerHTML +=
      '<p class="text-gray-500 text-sm">Chưa có dữ liệu nhân viên.</p>'
  }
...
function drawStoreContributionChart(storeEmployees, totalXP) {
  const container = document.getElementById(
    'store-contribution-chart-container'
  )
  container.innerHTML =
    '<h4 class="font-semibold mb-2">Tỷ lệ đóng góp XP</h4><canvas id="store-contribution-canvas"></canvas>'
  const ctx = document
    .getElementById('store-contribution-canvas')
    ?.getContext('2d')
  if (!ctx) return
  ...
  const topXP = topEmployees.reduce(
...
export async function init() {
  ...
  // Xử lý chuyển tab
  const employeeTab = document.querySelector(
    '.leaderboard-tab[data-tab="employees"]'
  )
  ...
  if (employees.length > 0) {
    ...
    const firstEmployeeCard = leaderboardListEmployee.querySelector(
      `[data-employee-id="${employees[0].id}"]`
    )
