...
const defaultColor = {
  ...
  tailwind_border: 'border-slate-400',
}
...
function renderGroupTabs() {
  ...
  if (allGroupedTasks.length === 0) {
    groupTabsContainer.innerHTML =
      '<p class="p-2 text-center text-xs text-slate-500">Không có nhóm nào.</p>'
  }
...
function updateTypeTaskCounts() {
  ...
  typeTabsContainer.querySelectorAll('.type-task-tab').forEach(tab => {
    ...
    const currentGroup = allGroupedTasks.find(
      g => g.id === currentFilters.groupId
    )
    let count = 0
...
function renderTypeTaskTabs() {
  ...
  typeTabsContainer.className =
    'flex-shrink-0 w-28 bg-slate-50 border-l border-slate-200 p-2 flex flex-col gap-2 overflow-y-auto'

  const types = [
    {
      id: 'Related',
      name: 'Gần đây',
      colorClasses: 'bg-indigo-100 text-indigo-800 hover:bg-indigo-200',
      icon: 'fas fa-history',
    },
    {
      id: 'CTM',
      name: 'CTM',
      colorClasses: 'bg-slate-200 text-slate-800 hover:bg-slate-300',
    },
    {
      id: 'Fixed',
      name: 'Fixed (F)',
  ...
  typeTabsContainer.innerHTML = types
    .map(type => {
      ...
      if (
        ...
        if (currentGroup && currentGroup.tasks) {
          count = currentGroup.tasks.filter(
            task => task.typeTask === type.id
          ).length
        }
      }
...
function renderTaskGrid() {
  ...
  if (currentFilters.typeTask === 'Related') {
    const searchTerm =
      document.getElementById('task-library-search')?.value.toLowerCase() || ''
  ...
  if (currentFilters.typeTask !== 'All') {
    filteredTasks = filteredTasks.filter(
      task => task.typeTask === currentFilters.typeTask
    )
  }
  ...
  if (filteredTasks.length > 0) {
    ...
    sortedTasks.forEach(task =>
      renderSingleTask({ ...task, groupId: group.id }, taskGridContainer)
    )
...
function loadMenuState() {
  ...
  if (savedState) {
    try {
      ...
      console.error(
        'Lỗi khi đọc trạng thái Task Library từ localStorage',
        error
      )
    }
  }
...
export async function initializeTaskLibrary() {
  ...
  const taskLibraryToggleArea = menuContainer.querySelector(
    '.task-library-toggle-area'
  )
  ...
  try {
    ...
    groupTabsContainer.innerHTML =
      '<p class="p-2 text-center text-xs text-red-500">Lỗi tải.</p>'
  }

  // --- Toggle expand/collapse ---
  // Sử dụng 'dblclick' để mở/đóng, tránh xung đột với sự kiện kéo-thả (mousedown/mouseup)
  taskLibraryToggleArea.addEventListener('dblclick', () => {
