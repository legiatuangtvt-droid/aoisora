...
export async function logTaskUsage(groupId, taskCode) {
  ...
  if (!currentUser || !currentUser.id || !groupId || !taskCode) {
    console.warn(
      '[Task Usage] Dừng ghi nhận: Thiếu thông tin người dùng, groupId, hoặc taskCode.'
    )
    return
  }
  ...
  try {
    ...
    await setDoc(
      ...
      { merge: true }
    ) // Dùng setDoc với merge:true và cấu trúc nested object
  } catch (error) {
...
export function addShiftRow(tbody, shiftNumber) {
  ...
  const workPositionOptions = allWorkPositions
    ? allWorkPositions
        .map(pos => `<option value="${pos.id}">${pos.name}</option>`)
        .join('')
    : ''
...
export async function updateTemplateFromDOM(
  ...
  manualShiftMappings = null,
  manualTotalManhour = null
) {
  const currentUser = window.currentUser
  ...
  if (!currentTemplateId && !manualScheduleData) {
    // Allow saving new template if manual data is provided
    return
  }
  ...
  const totalManhour =
    manualTotalManhour !== null
      ? manualTotalManhour
      : parseFloat(document.getElementById('template-manhour-input').value) || 0
  const hourlyManhours = {} // --- NEW: Đối tượng để lưu manhour theo giờ ---

  if (!manualScheduleData) {
    // If not provided manually, read from DOM
    document
  ...
  if (!manualShiftMappings) {
    // If not provided manually, read from DOM
    document
      .querySelectorAll('#template-builder-grid-container tbody tr')
      .forEach(row => {
        const shiftId = row.dataset.shiftId
        const selectedShiftCode = row.querySelector(
          '.shift-code-selector'
        )?.value
        const selectedPositionId = row.querySelector(
          '.work-position-selector'
        )?.value
        if (shiftId) {
  ...
  document
    .querySelectorAll('#template-builder-grid-container thead th[data-hour]')
    .forEach(headerCell => {
      ...
      const manhourValue =
        parseFloat(
          headerCell.querySelector('.hourly-position-count')?.textContent
        ) || 0
      if (hour) {
...
export async function loadTemplate(templateId) {
  ...
  const statusContainerForLoad = document.getElementById(
  ...
  if (templateData) {
    ...
    if (manhourDisplay)
      manhourDisplay.textContent = templateData.totalManhour || 0
  ...
  if (
    ...
    if (templateId && templateId !== 'new') {
      ...
      if (plansSnap.empty) {
        ...
        const statusText =
          planStatusMessages[plan.status] || `Trạng thái: ${plan.status}`
...
export async function deleteCurrentTemplate() {
  ...
  const confirmed = await showConfirmation(
    `Bạn có chắc chắn muốn xóa mẫu "${currentTemplate.name}" không? Hành động này không thể hoàn tác.`,
    'Xác nhận xóa',
    'Xóa',
    'Hủy'
  )
...
export function checkTemplateChangesAndToggleResetButton() {
  ...
  if (
    ...
    if (currentMonthlyPlan?.status === 'RM_AWAITING_APPROVAL') {
      ...
      deployButton.title =
        'Thay đổi vượt quá 10%, cần gửi HQ phê duyệt trước khi triển khai.'
    } else {
...
export async function handleResetTemplate() {
  const confirmed = await window.showConfirmation(
    'Bạn có chắc chắn muốn khôi phục lịch trình về trạng thái ban đầu do HQ gửi không? Mọi thay đổi sẽ bị mất.',
    'Xác nhận Reset',
    'Khôi phục',
    'Hủy'
  )
  if (confirmed) {
    ...
    checkTemplateChangesAndToggleResetButton()
    window.showToast(
      'Đã khôi phục lịch trình về trạng thái ban đầu.',
      'success'
    )
  }
}
...
export async function applyTemplateForHq() {
  ...
  if (
    ...
    if (!currentMonthlyPlan) {
      window.showToast(
        'Không tìm thấy kế hoạch tháng hiện tại để thực hiện hành động.',
        'error'
      )
      return
    }
    ...
    if (buttonText === 'Triển khai' || buttonText === 'Sẵn sàng triển khai') {
      ...
      try {
        ...
        const statusDisplay = document.getElementById('template-name-display')
        if (statusDisplay) {
          const cycleDate = currentMonthlyPlan.cycleStartDate.toDate
            ? currentMonthlyPlan.cycleStartDate.toDate()
            : new Date(currentMonthlyPlan.cycleStartDate)
          const newStatusText =
            planStatusMessages['RM_SENT_TO_STAFF'] || 'RM đã gửi về Staff'
  ...
  const selectedRegionIds = await window.showCheckboxListPrompt(
    ...
    'Áp dụng cho Miền',
    {
      ...
      rows: allRegions.map(region => {
        ...
        const regionalManager = allPersonnel.find(
          p =>
  ...
  const payrollSettingsRef = doc(
    db,
    'system_configurations',
    'payroll_settings'
  )
  const payrollSettingsSnap = await getDoc(payrollSettingsRef)
  const payrollStartDay = payrollSettingsSnap.exists()
    ? payrollSettingsSnap.data().startDay
    : 26
  ...
  const selectedRegionNames = selectedRegionIds
    .map(id => allRegions.find(r => r.id === id)?.name || id)
    .join(', ')
  ...
  await batch.commit()
  window.showToast(
    `Hoàn tất! Đã gửi kế hoạch từ mẫu "${template.name}" đến ${selectedRegionIds.length} miền đã chọn.`,
    'success',
    5000
  )
...
export async function loadAppliedPlanForManager() {
  ...
  if (currentUser.roleId === 'REGIONAL_MANAGER') {
    ...
    const areasSnap = await getDocs(
      query(
        collection(db, 'areas'),
        where('id', 'in', currentUser.managedAreaIds || ['dummy']),
        limit(1)
      )
    )
    if (!areasSnap.empty) {
  ...
  if (!regionIdToQuery) {
    ...
    document
      .getElementById('template-display-container')
      .querySelector('#template-name-display').textContent =
      'Bạn chưa được phân công vào miền nào.'
    renderGrid()
    return
  }
  ...
  if (plansSnap.empty) {
    ...
    document
      .getElementById('template-display-container')
      .querySelector('#template-name-display').textContent =
      'Chưa có kế hoạch nào được áp dụng cho miền của bạn.'
    renderGrid()
  } else {
    const allPlansForRegion = plansSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
...
export function updateTemplateStats() {
  ...
  const sortedGroupIds = Object.keys(allTaskGroups).sort(
    (a, b) => (allTaskGroups[a].order || 999) - (allTaskGroups[b].order || 999)
  )
  ...
  sortedGroupIds.forEach((groupId, index) => {
    ...
    if (!row) {
      const color =
        groupInfo.color && groupInfo.color.tailwind_text
          ? groupInfo.color
      ...
      row.querySelector('.stat-suggested').textContent =
        suggestedHours.toFixed(2)
  ...
  if (scheduledHoursValueEl)
    scheduledHoursValueEl.textContent = newTotalTime.toFixed(2)
}
 ...
 */
export async function handleAutoGenerate(
  openTime,
  closeTime,
  targetManHours,
  storeParams
  ...
  if (!currentTemplate) {
    throw new Error(
      'Không thể tìm thấy dữ liệu mẫu hiện tại để tiến hành tạo lịch.'
    )
  }
  ...
  const currentTemplateIndexForUpdate = allTemplates.findIndex(
  ...
  const currentTemplateIndex = allTemplates.findIndex(
    t => t.id === currentTemplateId
  )
  ...
  await updateTemplateFromDOM(
    generatedSchedule,
    generatedShiftMappings,
    budgetManHours
  )
}
