...
export async function fetchInitialData() {
  try {
    ...
    const areaManagers = areaManagersSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
    const regionalManagers = regionalManagersSnap.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }))
    ...
    if (container)
      container.innerHTML = `<div class="p-10 text-center text-red-500">Không thể tải dữ liệu vị trí công việc. Vui lòng thử lại.</div>`
  }
}
...
export async function renderMonthlyPlansForManager(plans) {
  ...
  const optionsHtml = plans
    .map(plan => {
      const cycleDate = plan.cycleStartDate.toDate
        ? plan.cycleStartDate.toDate()
        : new Date(plan.cycleStartDate)
