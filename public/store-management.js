...
async function fetchStoreStatuses() {
  ...
  allStoreStatuses = statusesSnapshot.docs.map(doc => ({
    id: doc.id,
    ...doc.data(),
  }))
  ...
  if (statusSelect) {
    statusSelect.innerHTML = allStoreStatuses
  ...
  const unsubscribe = onSnapshot(
    ...
    snapshot => {
      ...
      if (
        ...
        Array.isArray(currentUser.managedStoreIds)
      ) {
        // Chỉ giữ lại các cửa hàng mà SI này quản lý
        fetchedStores = fetchedStores.filter(store =>
          currentUser.managedStoreIds.includes(store.id)
        )
      }
    ...
    error => {
      console.error(
        '[Store] Lỗi khi lắng nghe thay đổi cửa hàng (onSnapshot):',
        error
      )
...
function renderStoreList(storeList) {
  ...
  storesForPage.forEach((store, index) => {
    const statusInfo = allStoreStatuses.find(s => s.id === store.status) || {
      name: store.status,
      color: 'gray',
    }
...
async function openStoreModal(storeId = null) {
  ...
  if (storeId) {
    ...
    modalTitle.innerHTML =
      '<i class="fas fa-store-alt mr-2 text-indigo-500"></i> Chỉnh Sửa Cửa Hàng'
    ...
    // Chế độ Thêm
    modalTitle.innerHTML =
      '<i class="fas fa-store-alt mr-2 text-indigo-500"></i> Thêm Cửa Hàng Mới'
...
export async function init() {
    ...
    storeForm.addEventListener(
      ...
      async e => {
        ...
        try {
          ...
          showToast(
            'Đã xảy ra lỗi khi lưu. Mã cửa hàng có thể đã tồn tại.',
            'error'
          )
        }
      },
  ...
  if (storeListTable) {
    storeListTable.addEventListener(
      'click',
      async e => {
        ...
        if (deleteBtn) {
          ...
          const confirmed = await showConfirmation(
            `Bạn có chắc chắn muốn xóa cửa hàng "${storeId}" không?`
          )
          if (confirmed) {
            try {
