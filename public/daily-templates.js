  ...
  document.querySelectorAll('.quarter-hour-slot').forEach(slot => {
    const sortable = Sortable.create(slot, {
      ...
      onAdd: evt => {
        ...
        const itemsInDest = Array.from(
          toSlot.querySelectorAll('.scheduled-task-item')
...
function logPosTaskCountForHour(hour, operation) {
  ...
  document
    .querySelectorAll(
      `#template-builder-tbody .quarter-hour-slot[data-time^="${hourKey}:"]`
    )
    .forEach(slot => {
      const task = slot.querySelector(
        `.scheduled-task-item[data-group-id="${posGroupId}"]`
      )
      if (task) {
        posTaskCount++
      }
  ...
  console.log(
    `[Drag&Drop Debug] Operation: ${operation} | Time: ${hour} | posTaskCount: ${posTaskCount}`
  )
}
...
function addControlsToTask(taskElement) {
  ...
  const isManager =
    window.currentUser &&
    (window.currentUser.roleId === 'REGIONAL_MANAGER' ||
      window.currentUser.roleId === 'AREA_MANAGER')
  ...
  if (!isManager && !taskElement.querySelector('.delete-task-btn')) {
    ...
    deleteBtn.className =
      'delete-task-btn absolute top-0 right-0 w-5 h-5 bg-red-500 text-white rounded-full text-xs opacity-0 group-hover:opacity-100 transition-opacity z-10'
...
function updateAllResizeHandlesVisibility() {
  const allTasks = document.querySelectorAll(
    '#template-builder-tbody .scheduled-task-item'
  )
  allTasks.forEach(task => {
    ...
    const getAdjacentSlot = (slot, direction) => {
      if (direction === 'previous') {
        ...
        return slot
          .closest('td')
          ?.nextElementSibling?.querySelector('.quarter-hour-slot:first-child')
      }
    }
    ...
    const hasTaskOnLeft =
      prevSlot && prevSlot.querySelector('.scheduled-task-item')
    leftHandle?.classList.toggle('hidden', !!hasTaskOnLeft)

    // Kiểm tra ô bên phải
    const nextSlot = getAdjacentSlot(currentSlot, 'next')
    const hasTaskOnRight =
      nextSlot && nextSlot.querySelector('.scheduled-task-item')
...
function initializeTaskCloning() {
  const gridContainer = document.getElementById(
    'template-builder-grid-container'
  )
  ...
  const onMouseUp = e => {
    ...
    ghostElement?.remove()
    document
      .querySelectorAll('.clone-target-highlight')
      .forEach(el => el.classList.remove('clone-target-highlight'))
  }
  ...
  const highlightTargetSlots = numSlots => {
    ...
    for (let i = 0; i < numSlots; i++) {
      ...
      const nextSlot =
        direction === 'right'
          ? currentSlot.nextElementSibling ||
            currentSlot.parentElement?.parentElement?.nextElementSibling?.querySelector(
              '.quarter-hour-slot'
            )
          : currentSlot.previousElementSibling ||
            currentSlot.parentElement?.parentElement?.previousElementSibling?.querySelector(
              '.quarter-hour-slot:last-child'
            )
      ...
      if (
        nextSlot &&
        !nextSlot.querySelector('.scheduled-task-item') &&
        !nextSlot.classList.contains('non-work-slot')
      ) {
  ...
  const cloneTasksToHighlightedSlots = () => {
    const highlightedSlots = document.querySelectorAll(
      '.clone-target-highlight'
    )
    highlightedSlots.forEach(slot => {
      ...
      newTask.classList.remove(
        'sortable-chosen',
        'sortable-ghost',
        'sortable-drag'
      ) // Xóa các class của SortableJS
    ...
    const affectedHours = new Set(
      Array.from(highlightedSlots).map(slot => slot.dataset.time)
    )
...
export async function init() {
  ...
  if (
    ...
    document
      .getElementById('template-selector-container')
    ...
    currentUser &&
    (currentUser.roleId === 'HQ_STAFF' || currentUser.roleId === 'ADMIN')
  ) {
    ...
    document
      ...
      ?.classList.remove('hidden')
    document
      .getElementById('template-display-container')
      ?.classList.remove('hidden') // Luôn hiển thị container trạng thái cho HQ
  ...
  document.getElementById('save-template-btn')?.addEventListener(
    'click',
    () => {
      /* Logic save đã chuyển đi */
    },
    { signal }
  )
  document
    .getElementById('new-template-btn')
    ?.addEventListener('click', switchToCreateNewMode, { signal })
  document
    .getElementById('delete-template-btn')
    ?.addEventListener('click', deleteCurrentTemplate, { signal })
  document
    .getElementById('reset-template-btn')
    ?.addEventListener('click', handleResetTemplate, { signal })
  document
    .getElementById('toggle-view-btn')
    ?.addEventListener('click', toggleBuilderView, { signal })
  ...
  document.getElementById('template-selector')?.addEventListener(
    'change',
    async e => {
    ...
    gridContainer.addEventListener(
      ...
      function (e) {
        ...
        if (deleteTaskButton) {
          ...
          if (taskItem) {
            // Kiểm tra an toàn để đảm bảo taskItem không phải là null
            taskItem.remove()
            updateTemplateFromDOM()
            updateTemplateStats()
        ...
        if (e.target.closest('.delete-shift-row-btn')) {
          ...
          window
            .showConfirmation(
              `Bạn có chắc chắn muốn xóa dòng ca này không?`,
              'Xác nhận xóa dòng',
              'Xóa',
              'Hủy'
            )
            .then(confirmed => {
              if (confirmed) {
                ...
                updateTemplateStats()
                updateGridHeaderStats()
              } // Giờ đây `row` chắc chắn không phải là null
            })
        }
  ...
  if (planTrackerModal && templateDisplayContainer) {
    ...
    planTrackerModal.addEventListener(
      'click',
      e => {
        if (
          ...
          planTrackerModal.addEventListener(
            'transitionend',
            () => planTrackerModal.classList.add('hidden'),
            { once: true }
          )
        }
      },
...
function initializeAutoGenerateModal(signal) {
  ...
  const distributionModal = document.getElementById(
    'customer-distribution-modal'
  )
  const openDistributionBtn = document.getElementById(
    'customize-customer-distribution-btn'
  )
  ...
  const openModal = () => {
    ...
    document.getElementById('ag-target-man-hours').value =
      currentTemplate?.totalManhour || '80'
    ...
    document.getElementById('ag-employee-count').value =
      storeInfo.employeeCount || '10'
    document.getElementById('ag-dry-goods-volume').value =
      storeInfo.dryGoodsVolume || '70'
    document.getElementById('ag-vegetable-weight').value =
      storeInfo.vegetableWeight || '20'

    // Tính và điền tổng số khách hàng từ dữ liệu theo giờ
    const customerCountByHour = storeInfo.customerCountByHour || {}
    const totalCustomers = Object.values(customerCountByHour).reduce(
      (sum, count) => sum + (parseInt(count, 10) || 0),
      0
    )
    document.getElementById('ag-total-customers').value =
      totalCustomers > 0 ? totalCustomers : '800'
  ...
  const closeModal = () => {
    ...
    autoGenerateModal.addEventListener(
      'transitionend',
  ...
  if (distributionModal && openDistributionBtn) {
    const distributionForm = document.getElementById(
      'customer-distribution-form'
    )
    const tableBody = document.getElementById('distribution-table-body')
    const totalPercentageSpan = document.getElementById(
    ...
    const updateDistributionTable = () => {
      ...
      const totalCustomers =
        parseInt(document.getElementById('ag-total-customers').value, 10) || 0
      ...
      tableBody.querySelectorAll('tr').forEach(row => {
        ...
        const calculatedCustomers = Math.round(
          totalCustomers * (percentage / 100)
        )
        customerCountCell.textContent =
          calculatedCustomers.toLocaleString('vi-VN')
      })
    ...
    const closeDistributionModal = () => {
      ...
      distributionModal.addEventListener(
        'transitionend',
    ...
    openDistributionBtn.addEventListener('click', openDistributionModal, {
      signal,
    })
    distributionModal
      .querySelector('.modal-close-btn')
      .addEventListener('click', closeDistributionModal, { signal })
    distributionModal
  ...
  if (autoGenerateForm) {
    autoGenerateForm.addEventListener(
      'submit',
      async event => {
        ...
        if (!templateId) {
          const newTemplateName = await window.showPrompt(
            'Bạn đang tạo lịch cho một mẫu mới. Vui lòng đặt tên cho mẫu này:',
            'Đặt tên cho Mẫu mới',
            'Auto Generate Model'
          )
        ...
        const targetManHours = parseFloat(
          document.getElementById('ag-target-man-hours').value
        )
        const totalCustomers =
          parseInt(document.getElementById('ag-total-customers').value, 10) || 0
        ...
        for (const hour in customerDistributionProfile) {
          ...
          newCustomerCountByHour[hour] = Math.round(
            totalCustomers * (percentage / 100)
          )
        }
        ...
        const storeParams = {
          ...
          dryGoodsVolume: parseInt(
            document.getElementById('ag-dry-goods-volume').value,
            10
          ),
          vegetableWeight: parseInt(
            document.getElementById('ag-vegetable-weight').value,
            10
          ),
        ...
        const submitBtn = autoGenerateForm.querySelector(
          'button[type="submit"]'
        )
