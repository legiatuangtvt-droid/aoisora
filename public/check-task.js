...
async function initializeCheckTaskView() {
  ...
  if (!tableContainer) {
    ...
    tableContainer.className =
      'bg-white rounded-lg shadow-md overflow-hidden p-4' // Thêm style và margin-top
...
async function renderTaskSummaryTable(container) {
  ...
  try {
    ...
    const mockHours = {
      POS: { model: 20.0, dws: 21.25 },
      PERI: { model: 14.5, dws: 13.75 },
      DRY: { model: 10.75, dws: 12.25 },
      MMD: { model: 6.5, dws: 6.0 },
      LEADER: { model: 8.0, dws: 8.0 },
      'QC-FSH': { model: 9.5, dws: 8.75 },
      DELICA: { model: 3.0, dws: 3.75 },
      'D&D': { model: 2.0, dws: 2.75 },
      OTHER: { model: 5.75, dws: 4.5 },
    }
    ...
    days.forEach((_, dayIndex) => {
      ...
      taskGroups.forEach(group => {
        ...
        if (groupsWithGap1.some(g => g.code === groupCode)) {
          ...
          dwsData[groupCode][dayIndex] = Math.max(
            0,
            modelData[groupCode][dayIndex] + variation
          )
        } else {
        ...
        if (groupsWithGap2.some(g => g.code === groupCode)) {
          const variation = (Math.floor(Math.random() * 3) - 1) * 0.25 // -0.25, 0, 0.25
          actualData[groupCode][dayIndex] = Math.max(
            0,
            modelData[groupCode][dayIndex] + variation
          )
        } else {
    ...
    dwsRow.innerHTML =
      `<td class="px-5 py-3 border border-black bg-white text-sm font-bold text-center">DWS</td>` +
      days
        .map((date, index) => {
          const modelTotal = Object.values(modelData).reduce(
            (sum, groupData) => sum + groupData[index],
            0
          )
          const dwsTotal = Object.values(dwsData).reduce(
            (sum, groupData) => sum + groupData[index],
            0
          )
    ...
    modelRow.innerHTML =
      `<td class="px-5 py-3 border border-black bg-white text-sm font-bold text-center">Model</td>` +
      days
        .map((date, index) => {
          const modelTotal = Object.values(modelData).reduce(
            (sum, groupData) => sum + groupData[index],
    ...
    taskGroups.forEach(group => {
      ...
      groupRow.innerHTML =
        `<td class="px-5 py-3 border border-black bg-white text-sm text-center">${group.code}</td>` +
        days
          .map((date, index) => {
            ...
            const gap1HTML =
              gap1 !== 0
                ? `<div class="absolute top-2 right-0.5 text-[8px] font-semibold ${colorClass1}" title="DWS - Model">
                                        GAP 1: ${Math.abs(gap1).toFixed(2)} ${indicator1}
                                    </div>`
                : ''
            const gap2HTML =
              gap2 !== 0
                ? `<div class="absolute bottom-2 right-0.5 text-[8px] font-semibold ${colorClass2}" title="Actual - Model">
    ...
    gap1Row.innerHTML =
      `<td class="px-5 py-3 border border-black bg-white text-sm font-bold text-center">GAP 1</td>` +
      days
        .map((date, index) => {
          const modelTotal = Object.values(modelData).reduce(
            (sum, groupData) => sum + groupData[index],
    ...
    gap2Row.innerHTML =
      `<td class="px-5 py-3 border border-black bg-white text-sm font-bold text-center">GAP 2</td>` +
      days
        .map((date, index) => {
          const modelTotal = Object.values(modelData).reduce(
            (sum, groupData) => sum + groupData[index],
          ...
          const actualTotal = Object.values(actualData).reduce(
            (sum, groupData) => sum + groupData[index],
            0
          )
    ...
    notesContainer.className =
      'mt-2 p-2 bg-slate-50 rounded-lg border border-slate-200 text-sm'
...
async function fetchTaskGroups() {
  ...
  const taskGroupsSnapshot = await getDocs(
    query(collection(db, 'task_groups'), orderBy('order'))
  )
