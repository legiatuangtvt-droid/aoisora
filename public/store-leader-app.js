...
import { initializeTaskLibrary } from './task-library.js'
...
const storeLeaderPages = {
  ...
  'monthly-schedules.html': {
    module: monthlySchedules,
    title: 'Lịch Làm Việc Tháng',
  },
  'store-availability-assignment.html': {
    module: storeAvailabilityAssignment,
...
async function initializeStoreLeaderApp() {
  ...
  const initialPageName =
    window.location.pathname.split('/').pop() || 'monthly-schedules.html'
  loadPageModule(initialPageName)

  document.addEventListener('page-content-loaded', event =>
    loadPageModule(event.detail.pageName)
  )
}
