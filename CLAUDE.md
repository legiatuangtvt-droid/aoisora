...
### 12.4.1 Parent-Child Task Relationship (Mối quan hệ Task Cha-Con)

> **Quan trọng**: Section này mô tả logic giữa Parent Task (Level 1) và Child Tasks (Level 2, 3, 4, 5)
...
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📊 CẤU TRÚC PHÂN CẤP (Task Hierarchy)                          │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│     Level 1: PARENT TASK (Task Cha)                            │
│     ├── Level 2: Sub-task (Task Con cấp 1)                     │
│     │   ├── Level 3: Sub-sub-task (Task Con cấp 2)             │
│     │   │   ├── Level 4: ...                                   │
│     │   │   └── Level 5: ... (Max depth)                       │
│     │   └── Level 3: ...                                       │
│     └── Level 2: ...                                           │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  🎯 NGUYÊN TẮC CHÍNH: Parent Task ĐẠI DIỆN cho toàn bộ Children │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  1️⃣ STATUS (Trạng thái)                                     ││
│  │  ─────────────────────────────────────────────────────────  ││
│  │                                                             ││
│  │  • Status CHỈ thuộc về PARENT TASK                          ││
│  │  • Sub-tasks KHÔNG có status riêng                          ││
│  │  • Khi Approve → Approve cả PARENT + tất cả Children        ││
│  │  • Khi Reject → Reject cả PARENT + tất cả Children          ││
│  │                                                             ││
│  │  Ví dụ: Task "Kiểm kê kho tháng 1" có 3 sub-tasks:          ││
│  │  - Kiểm kê khu thực phẩm                                    ││
│  │  - Kiểm kê khu hóa mỹ phẩm                                  ││
│  │  - Kiểm kê khu đồ gia dụng                                  ││
│  │                                                             ││
│  │  → Approver chỉ cần Approve 1 lần (task cha)                ││
│  │  → TẤT CẢ 4 tasks (1 cha + 3 con) đều được Approve          ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  2️⃣ HQ CHECK (Kiểm tra HQ)                                   ││
│  │  ─────────────────────────────────────────────────────────  ││
│  │                                                             ││
│  │  • HQ Check CHỈ áp dụng cho PARENT TASK                     ││
│  │  • Sub-tasks KHÔNG có HQ Check riêng                        ││
│  │  • Khi HQ Check parent → coi như check cả group             ││
│  │                                                             ││
│  │  UI hiển thị:                                               ││
│  │  - Parent task: Hiển thị HQ Check status (Not Yet/Done)     ││
│  │  - Sub-tasks: Cột HQ Check để TRỐNG (không có badge)        ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  3️⃣ PROGRESS & UNABLE (Tiến độ & Không thể thực hiện)       ││
│  │  ─────────────────────────────────────────────────────────  ││
│  │                                                             ││
│  │  • Progress/Unable tính theo STORE ASSIGNMENTS              ││
│  │  • KHÔNG tính theo số lượng sub-tasks                       ││
│  │  • Mỗi Store có 1 assignment cho PARENT TASK                ││
│  │  • Khi Store báo done → nghĩa là hoàn thành CẢ GROUP        ││
│  │                                                             ││
│  │  Cách tính:                                                 ││
│  │  - Progress = done / total_stores (không phải sub-tasks)    ││
│  │  - Unable = số stores báo unable                            ││
│  │                                                             ││
│  │  UI hiển thị:                                               ││
│  │  - Parent task: Hiển thị Progress (0/3) và Unable (0)       ││
│  │  - Sub-tasks: Cột Progress và Unable để TRỐNG               ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  ⚠️ QUY TẮC STORE BÁO DONE                                       │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  Để Store có thể báo "Done", PHẢI thỏa mãn:                     │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  ✅ ĐIỀU KIỆN BÁO DONE:                                      ││
│  │                                                             ││
│  │  1. Parent Task đã được thực hiện                           ││
│  │  2. TẤT CẢ Sub-tasks (nếu có) đã được thực hiện             ││
│  │  3. Tất cả evidence (nếu required) đã được upload           ││
│  │                                                             ││
│  │  → Chỉ khi CẢ 3 điều kiện thỏa mãn mới cho phép báo Done    ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  ❌ KHÔNG CHO PHÉP BÁO DONE KHI:                             ││
│  │                                                             ││
│  │  • Chỉ hoàn thành Parent task, sub-tasks chưa làm           ││
│  │  • Chỉ hoàn thành một số sub-tasks, còn lại chưa làm        ││
│  │  • Parent task chưa hoàn thành dù sub-tasks đã xong         ││
│  │                                                             ││
│  │  Ví dụ: Task "Kiểm kê kho tháng 1" có 3 sub-tasks           ││
│  │  - Store A hoàn thành Parent + 2/3 sub-tasks                ││
│  │  → KHÔNG cho phép báo Done (thiếu 1 sub-task)               ││
│  │                                                             ││
│  │  - Store B hoàn thành 3/3 sub-tasks nhưng chưa làm Parent   ││
│  │  → KHÔNG cho phép báo Done (thiếu Parent)                   ││
│  │                                                             ││
│  │  - Store C hoàn thành Parent + 3/3 sub-tasks                ││
│  │  → ✅ Cho phép báo Done                                     ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📋 BẢNG TÓM TẮT: Thuộc tính theo Level                         │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  ┌────────────────┬───────────┬───────────────────────────────┐ │
│  │ Thuộc tính     │ Parent    │ Sub-tasks (Level 2-5)         │ │
│  │                │ (Level 1) │                               │ │
│  ├────────────────┼───────────┼───────────────────────────────┤ │
│  │ Status         │ ✅ Có     │ ❌ Không (kế thừa từ Parent)  │ │
│  │ HQ Check       │ ✅ Có     │ ❌ Không (kế thừa từ Parent)  │ │
│  │ Progress       │ ✅ Có     │ ❌ Không (tính từ stores)     │ │
│  │ Unable         │ ✅ Có     │ ❌ Không (tính từ stores)     │ │
│  │ Start-End Date │ ✅ Có     │ ✅ Có (kế thừa hoặc override) │ │
│  │ Task Name      │ ✅ Có     │ ✅ Có                         │ │
│  │ Task Type      │ ✅ Có     │ ✅ Có (theo hierarchy rule)   │ │
│  │ Store Assign   │ ✅ Có     │ ❌ Không (Parent level only)  │ │
│  └────────────────┴───────────┴───────────────────────────────┘ │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  🖥️ UI DISPLAY RULES (Quy tắc hiển thị)                          │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  Task List Table:                                               │
│  ┌──────┬──────┬───────────────┬─────────┬────────┬────────┬────────┬──────────┐
│  │ No   │ Dept │ Task Group    │ Start-  │Progress│ Unable │ Status │ HQ Check │
│  │      │      │               │ End     │        │        │        │          │
│  ├──────┼──────┼───────────────┼─────────┼────────┼────────┼────────┼──────────┤
│  │ 1    │ OP   │ Kiểm kê kho   │ 24/01-  │  0/3   │   0    │Approve │ Not Yet  │
│  │      │      │ tháng 1    ▼  │ 27/01   │        │        │        │          │
│  ├──────┼──────┼───────────────┼─────────┼────────┼────────┼────────┼──────────┤
│  │      │      │   └ Kiểm kê   │ 24/01-  │        │        │Approve │          │
│  │      │      │     khu thực  │ 27/01   │        │        │        │          │
│  │      │      │     phẩm      │         │        │        │        │          │
│  ├──────┼──────┼───────────────┼─────────┼────────┼────────┼────────┼──────────┤
│  │      │      │   └ Kiểm kê   │ 24/01-  │        │        │Approve │          │
...
│                                                                 │
│  Legend:                                                        │
│  • Parent row: Hiển thị đầy đủ tất cả columns                   │
│  • Sub-task rows: No, Dept, Progress, Unable, HQ Check = TRỐNG  │
│  • Sub-task rows: Task Group có indent, Status kế thừa Parent   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| O.16 | Empty state | Filter/Search không có kết quả → Empty UI | ⏳ | ⏳ | - |
