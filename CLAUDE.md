...
### 12.8 Store Task Execution Flow (Giai đoạn Store thực hiện Task)

```
┌─────────────────────────────────────────────────────────────────┐
│  STORE TASK EXECUTION FLOW                                      │
│                                                                 │
...
│     │  │ → KHÔNG thể Mark Unable (chỉ S2+ mới được)       │   │ │
│     │  └──────────────────────────────────────────────────┘   │ │
│     │                                                         │ │
│     └─────────────────────────────────────────────────────────┘ │
│                                                                 │
...
│        → Có quyền Start, Complete, Upload Evidence             │
│        → KHÔNG có quyền Mark Unable                            │
│                                                                 │
│     🔄 REASSIGN / UNASSIGN:                                      │
│                                                                 │
│     ┌─────────────────────────────────────────────────────────┐ │
...
│     ──────────────────────────────────────────────────────────  │
│     Trigger: Store Leader click "Unable to Complete" button    │
│     Conditions:                                                │
│       • Current store_status = 'not_yet' OR 'on_progress'      │
│       • CHỈ Store Leaders (S4/S3/S2) có quyền                  │
│       • S1 KHÔNG được phép mark unable                         │
...
| Mark Unable | POST | `/api/v1/tasks/{id}/stores/{store_id}/unable` | Đánh dấu không thể làm | S4-S2 only (Store Leaders) |
...
**Permission Logic cho Start/Complete:**

```
ON START or COMPLETE action:
...
ON MARK UNABLE action:
   → ALLOW only if current_user.job_grade IN (S4, S3, S2)
   → S1 KHÔNG BAO GIỜ được mark unable
```
...
### 12.9 Task Execution History (Lịch sử thực hiện Task)
