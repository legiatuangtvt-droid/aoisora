...
### 12.1.1 Task Parent-Child Form Rules (Quy tắc Form cho Task Cha-Con)

> **Nguyên tắc chính**:
> - Task CHA (parent - có con): Chỉ chứa thông tin tổng thể, TIÊU ĐỀ
> - Task CON (child - leaf node): Chứa thông tin chi tiết công việc
> - Chỉ task CON (leaf node) mới cần HQ Check/Confirm
> - Khi task có con → B.Instructions bị lock/ẩn

```
┌─────────────────────────────────────────────────────────────────┐
│  TASK PARENT-CHILD FORM RULES                                   │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📌 NGUYÊN TẮC CƠ BẢN:                                           │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  • Task CHA (parent): Chỉ nắm giữ TIÊU ĐỀ và thông tin tổng thể │
│  • Task CON (child): Nắm giữ thông tin YÊU CẦU CÔNG VIỆC chi tiết│
│  • Task level số NHỎ hơn = Task CHA (nắm giữ task level lớn hơn)│
│  • Task level số LỚN hơn = Task CON (chứa detail)               │
│  • Chỉ LEAF NODE (task không có con) mới cần HQ Check           │
│                                                                 │
│  Ví dụ: Task Level 1 có nhiều Task Level 2                      │
│         → Level 1 chỉ có tiêu đề, thông tin tổng quát           │
│         → Level 2 chứa Instructions chi tiết                    │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📋 TASK CHA (Parent - có ít nhất 1 task con)                    │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ A. INFORMATION                                              ││
│  ├─────────────────────────────────────────────────────────────┤│
│  │ • Task Type      : ✅ Được phép CHỌN                        ││
│  │ • Applicable Period:                                        ││
│  │   - Start Date   : ≤ Start của TẤT CẢ task con              ││
│  │   - End Date     : ≥ End của TẤT CẢ task con                ││
│  │   - ⚠️ Approve PHẢI thực hiện TRƯỚC start date (ràng buộc)  ││
│  │ • RE Time        : ≥ TỔNG RE của tất cả task con cộng lại   ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ B. INSTRUCTIONS (LOCKED/EMPTY khi có con)                   ││
│  ├─────────────────────────────────────────────────────────────┤│
│  │ • Task Type      : ⚪ Hiển thị RỖNG, không cho phép chọn    ││
│  │ • Manual Link    : ⚪ Hiển thị RỖNG, không cho phép chọn    ││
│  │ • Note           : ✅ Được phép NHẬP                        ││
│  │ • Photo Guidelines: 🔴 ẨN đi hoàn toàn                      ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ C. SCOPE         : ✅ Hiển thị đầy đủ                       ││
│  │ D. APPROVAL      : ✅ Hiển thị đầy đủ                       ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📋 TASK CON (Child - leaf node, không có task con)              │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ A. INFORMATION                                              ││
│  ├─────────────────────────────────────────────────────────────┤│
│  │ • Task Type      : ❌ Chỉ HIỂN THỊ theo task cha, không chọn││
│  │ • Applicable Period:                                        ││
│  │   - Start Date   : ≥ Start của task cha                     ││
│  │   - End Date     : ≤ End của task cha                       ││
│  │ • RE Time        : ≤ RE của task cha                        ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ B. INSTRUCTIONS (FULL ACCESS cho leaf node)                 ││
│  ├─────────────────────────────────────────────────────────────┤│
│  │ • Task Type      : ✅ Được phép CHỌN (Image/Document)       ││
│  │ • Manual Link    : ✅ Được phép NHẬP                        ││
│  │ • Note           : ✅ Được phép NHẬP                        ││
│  │ • Photo Guidelines: ✅ Được phép UPLOAD (nếu Task Type=Image)││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ C. SCOPE         : 🔴 ẨN đi (kế thừa từ task cha)           ││
│  │ D. APPROVAL      : 🔴 ẨN đi (kế thừa từ task cha)           ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  🔄 DYNAMIC CONVERSION (Chuyển đổi động khi thêm task con)       │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  Khi task đã có Instructions đầy đủ → Thêm task con:            │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ BEFORE: Task Level 2 (leaf node - có Instructions)          ││
│  │         B.Instructions: Task Type=Image, Manual Link=URL,   ││
│  │                         Photo Guidelines có ảnh             ││
│  │                                                             ││
│  │ ACTION: User bấm [+] thêm Task Level 3                      ││
│  │                                                             ││
│  │ AFTER:  Task Level 2 TRỞ THÀNH PARENT                       ││
│  │         B.Instructions: Task Type=RỖNG, Manual Link=RỖNG,   ││
│  │                         Photo Guidelines=ẨN                 ││
│  │                         Note=GIỮ NGUYÊN                     ││
│  │                                                             ││
│  │         Task Level 3 MỚI (leaf node)                        ││
│  │         B.Instructions: EMPTY (user nhập mới)               ││
│  │         → Thông tin Instructions cũ của Level 2 bị XÓA      ││
│  │         → KHÔNG tự động copy xuống Level 3                  ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ⚠️ QUAN TRỌNG:                                                  │
│  • Khi task trở thành parent → Instructions cũ BỊ XÓA          │
│  • KHÔNG tự động chuyển Instructions xuống task con            │
│  • User phải nhập lại Instructions cho task con mới            │
│  • Note là field DUY NHẤT được giữ lại ở task cha              │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  ✅ HQ CHECK / CONFIRM                                           │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  • Chỉ áp dụng cho LEAF NODE (task không có con)               │
│  • Task cha KHÔNG cần HQ Check (chỉ validate Instructions)     │
│  • Store thực hiện và báo cáo kết quả cho LEAF NODE            │
│  • Overall status của task cha = tổng hợp từ tất cả leaf nodes │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**So sánh nhanh Task Parent vs Child:**

| Section | Field | Task CHA (có con) | Task CON (leaf) |
|---------|-------|-------------------|-----------------|
| **A. Info** | Task Type | ✅ Được chọn | ❌ Readonly (theo cha) |
| **A. Info** | Start Date | ≤ start con | ≥ start cha |
| **A. Info** | End Date | ≥ end con | ≤ end cha |
| **A. Info** | RE Time | ≥ tổng RE con | ≤ RE cha |
| **B. Instructions** | Task Type | ⚪ Rỗng, locked | ✅ Được chọn |
| **B. Instructions** | Manual Link | ⚪ Rỗng, locked | ✅ Được nhập |
| **B. Instructions** | Note | ✅ Được nhập | ✅ Được nhập |
| **B. Instructions** | Photo Guidelines | 🔴 Ẩn | ✅ Hiển thị |
| **C. Scope** | | ✅ Hiển thị | 🔴 Ẩn |
| **D. Approval** | | ✅ Hiển thị | 🔴 Ẩn |
| **HQ Check** | | ❌ Không cần | ✅ Cần confirm |

**Validation Rules:**

| Rule | Condition | Error Message |
|------|-----------|---------------|
| Parent Start ≤ Child Start | `parent.start_date <= child.start_date` | "Start date must be on or after parent's start date" |
| Parent End ≥ Child End | `parent.end_date >= child.end_date` | "End date must be on or before parent's end date" |
| Parent RE ≥ Sum(Child RE) | `parent.re_time >= SUM(children.re_time)` | "RE Time must be at least the sum of all child tasks" |
| Child RE ≤ Parent RE | `child.re_time <= parent.re_time` | "RE Time cannot exceed parent's RE Time" |
| Approve before Start | `approved_at < start_date` | "Task must be approved before start date" |
...
### 12.4.1 Parent-Child Task Status Calculation (Tính toán Status Task Cha-Con)

> **Quan trọng**: Section này mô tả logic tính Overall Status của Parent Task từ Self Status của Parent và tất cả Descendants.
>
> **Approach**: Flat Aggregation - Tổng hợp tất cả self_status một lần để tính overall_status.

```
┌─────────────────────────────────────────────────────────────────┐
│  PARENT-CHILD STATUS CALCULATION                                │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  📊 KHÁI NIỆM: 2 LOẠI STATUS                                    │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  Mỗi task (Parent hoặc Child) có:                               │
│                                                                 │
│  1. SELF STATUS (Status riêng)                                  │
│     → Trạng thái thực hiện của chính task đó                    │
│     → Giá trị: not_yet, on_progress, done, unable, overdue      │
│                                                                 │
│  2. OVERALL STATUS (Status tổng thể) - Chỉ Parent có            │
│     → Tổng hợp từ: Self Status của Parent + tất cả Descendants  │
│     → Giá trị: NOT_YET, ON_PROGRESS, DONE, OVERDUE              │
│     → Hiển thị trên Task List (HQ view)                         │
│                                                                 │
│  ═══════════════════════════════════════════════════════════════│
│  🔄 FLAT AGGREGATION (Tổng hợp một lần)                         │
│  ═══════════════════════════════════════════════════════════════│
│                                                                 │
│  Input: Tất cả self_status (Parent + ALL Descendants)           │
│                                                                 │
│  Level 1 overall = aggregate(                                   │
│      Level 1 self,                                              │
│      Level 2 self (all),                                        │
│      Level 3 self (all),                                        │
│      Level 4 self (all),                                        │
...
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│  VÍ DỤ 1: Task 5 levels - có 1 task overdue                                     │
│                                                                                 │
│  📋 ROOT (L1)                                                                   │
│   ├── 📋 Task 1 (L2) ─────── self: done                                        │
│   │    ├── 📋 Task 1.1 (L3) ─ self: done                                       │
│   │    │    └── 📋 Task 1.1.1 (L4) ─ self: 🔴 overdue                          │
│   │    └── 📋 Task 1.2 (L3) ─ self: done                                       │
│   └── 📋 Task 2 (L2) ─────── self: done                                        │
│                                                                                 │
│  All statuses = [done, done, done, overdue, done, done]                         │
│  → Có overdue? ✅ YES → Overall = 🔴 OVERDUE                                    │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  VÍ DỤ 2: Task 5 levels - có task đang làm                                      │
│                                                                                 │
│  📋 ROOT (L1)                                                                   │
│   ├── 📋 Task 1 (L2) ─────── self: done                                        │
│   │    ├── 📋 Task 1.1 (L3) ─ self: 🔵 on_progress                             │
│   │    └── 📋 Task 1.2 (L3) ─ self: not_yet                                    │
│   └── 📋 Task 2 (L2) ─────── self: done                                        │
│                                                                                 │
│  All statuses = [done, done, on_progress, not_yet, done]                        │
│  → Có overdue? ❌ NO                                                            │
│  → Có on_progress? ✅ YES → Overall = 🔵 ON_PROGRESS                            │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  VÍ DỤ 3: Task 5 levels - tất cả hoàn thành                                     │
│                                                                                 │
│  📋 ROOT (L1)                                                                   │
│   ├── 📋 Task 1 (L2) ─────── self: done                                        │
│   │    ├── 📋 Task 1.1 (L3) ─ self: done                                       │
│   │    └── 📋 Task 1.2 (L3) ─ self: unable                                     │
│   └── 📋 Task 2 (L2) ─────── self: done                                        │
│                                                                                 │
│  All statuses = [done, done, done, unable, done]                                │
│  → Có overdue? ❌ NO                                                            │
│  → Có on_progress? ❌ NO                                                        │
│  → Tất cả done/unable? ✅ YES → Overall = 🟢 DONE                               │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  VÍ DỤ 4: Task 5 levels - chưa ai bắt đầu                                       │
│                                                                                 │
│  📋 ROOT (L1)                                                                   │
│   ├── 📋 Task 1 (L2) ─────── self: not_yet                                     │
│   │    └── 📋 Task 1.1 (L3) ─ self: not_yet                                    │
│   └── 📋 Task 2 (L2) ─────── self: not_yet                                     │
│                                                                                 │
│  All statuses = [not_yet, not_yet, not_yet, not_yet]                            │
│  → Có overdue? ❌ NO                                                            │
│  → Có on_progress? ❌ NO                                                        │
│  → Tất cả done/unable? ❌ NO                                                    │
│  → Tất cả not_yet? ✅ YES → Overall = 🟡 NOT_YET                                │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
...
```typescript
// TypeScript
type SelfStatus = 'not_yet' | 'on_progress' | 'done' | 'unable' | 'overdue';
